...
# 4. ADD WARNINGS
# The review said doneCount caused ReferenceError.
# Wait, doneCount IS defined:
# const doneCount=useMemo(()=>{if(!done)return 0;let c=0;for(let i=0;i<done.length;i++)if(done[i])c++;return c;},[done]);
# But in `handlePatClick`, there's no reference to `doneCount` natively. I added it. Is `doneCount` out of scope?
# `handlePatClick` is defined inside `App`, so `doneCount` is in scope.
# Ah, wait! The error could have been in `generate()`!
# `generate` is wrapped in `useCallback`. Does `generate` have `doneCount` in its dependency array?
# No! `generate` has:
# `},[img,sW,sH,maxC,bri,con,sat,dith,skipBg,bgCol,bgTh,minSt,smooth,smoothType,orphans,hasGenerated]);`
# So if I use `doneCount` inside `generate`, I must add `doneCount` to the dependency array. BUT that would cause `generate` to be recreated on every stitch!
# Instead, I can just calculate `doneCount` dynamically, or use a ref, or just check `done` directly!
# A much safer check inside `generate`, `autoCrop`, and `handlePatClick`:
...
func_paint = """if((activeTool==="paint"||activeTool==="fill")&&selectedColorId&&cmap){
    """
content = content.replace(func_paint, func_paint + warn_cond + "\n    ")

func_erasebs = """if(activeTool==="eraseBs"){
    """
content = content.replace(func_erasebs, func_erasebs + warn_cond + "\n    ")

func_backstitch = """if(activeTool==="backstitch"){if(gx<0||gx>sW||gy<0||gy>sH)return;"""
content = content.replace(func_backstitch, """if(activeTool==="backstitch"){if(gx<0||gx>sW||gy<0||gy>sH)return;""" + warn_cond + " ")
