...
function EmbroideryApp(){
  ...
  const handleFile=useCallback(e=>{const f=e.target.files?.[0];if(!f)return;const reader=new FileReader();reader.onload=ev=>{setImgSrc(ev.target.result);const img=new Image();img.onload=()=>{imgRef.current=img;const ic=imgC.current;ic.width=CW;ic.height=CH;const ctx=ic.getContext("2d");ctx.clearRect(0,0,CW,CH);const s=Math.min(CW/img.width,CH/img.height);ctx.drawImage(img,(CW-img.width*s)/2,(CH-img.height*s)/2,img.width*s,img.height*s);imgDataRef.current=ctx.getImageData(0,0,CW,CH);setPhase("segment");};img.onerror=()=>{imgRef.current=null;imgDataRef.current=null;try{window.alert("Could not decode image. The file may be corrupt or in an unsupported format.");}catch(_){}};img.src=ev.target.result;};reader.onerror=()=>{imgRef.current=null;imgDataRef.current=null;try{window.alert("Could not read file: "+(reader.error&&reader.error.message?reader.error.message:"unknown error"));}catch(_){}};reader.onabort=()=>{imgRef.current=null;imgDataRef.current=null;};reader.readAsDataURL(f);},[]);
