...
function drawPattern(ctx,offX,offY,dW,dH,cSz,gut, viewportRect=null, showOverlayImg=false, op=0.3){
  ...
  for(let y2=startY;y2<endY;y2++)for(let x2=startX;x2<endX;x2++){let idx=(offY+y2)*sW+(offX+x2);let m=pat[idx];if(!m)continue;let px=gut+x2*cSz,py=gut+y2*cSz;let isHi=!hiId||m.id===hiId;let dim=hiId&&!isHi&&m.id!=="__skip__";
    if(m.id==="__skip__"){
      if(showOverlayImg){
...
function handlePatClick(e){
  ...
  if((activeTool==="paint"||activeTool==="fill")&&selectedColorId&&cmap){
    ...
    const applyStitch = (existingCell, newPe, newStitchType) => {
      ...
      // Allow overriding secondary cells directly if they exist
      if (cell.secondary) {
