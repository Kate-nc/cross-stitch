...
const[palAdvanced,setPalAdvanced]=useState(false);
...
const generatePreview=useCallback(()=>{
  ...
  const confettiRaw=analyzeConfetti(mapped,pw,ph);
  if(orphans>0)mapped=removeOrphanStitches(mapped,pw,ph,orphans);
  const confettiClean=orphans>0?analyzeConfetti(mapped,pw,ph):null;
  ...
  setPreviewStats({
    ...
    confettiTotal: Math.round(confettiRaw.total*scaleFactor),
...
const autoCrop = useCallback(() => {
  if(done && done.some(v => v===1)) { if(!confirm("This pattern has tracking progress. Editing it will reset your progress. Continue?")) return; }
  if (!pat || !img) return;
...
const generate=useCallback(()=>{
  ...
  setTimeout(()=>{try{
    ...
    const rawConfetti=analyzeConfetti(mapped,sW,sH);
    if(orphans>0)mapped=removeOrphanStitches(mapped,sW,sH,orphans);
    const cleanConfetti=orphans>0?analyzeConfetti(mapped,sW,sH):rawConfetti;
    setConfettiData({raw:rawConfetti,clean:cleanConfetti});
...
<div className="cs-page-content" style={{maxWidth:1100,margin:"0 auto",padding:"20px 16px"}}>
  ...
  {img&&<div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
    {tab!=="stitch"&&<div style={{flex:sidebarOpen?"0 0 280px":"0 0 auto",display:"flex",flexDirection:"column",gap:sidebarOpen?10:0,overflow:"hidden"}}>
      ...
      <Section title="Palette" isOpen={palOpen} onToggle={setPalOpen}><div style={{marginTop:8}}><SliderRow label="Max skeins" value={maxC} min={10} max={40} onChange={setMaxC}/></div><div style={{marginTop:8}}><SliderRow label="Min stitches/colour" value={minSt} min={0} max={50} onChange={setMinSt} format={v=>v===0?"Off":v}/></div><button onClick={()=>setPalAdvanced(o=>!o)} style={{marginTop:8,display:"flex",alignItems:"center",gap:4,fontSize:11,color:"#71717a",background:"none",border:"none",cursor:"pointer",padding:"2px 0",fontFamily:"inherit"}}><span style={{fontSize:9,display:"inline-block",transform:palAdvanced?"rotate(90deg)":"rotate(0deg)",transition:"transform 0.15s"}}>▶</span>Advanced{(orphans>0||dith)?<span style={{width:6,height:6,borderRadius:"50%",background:"#0d9488",display:"inline-block",marginLeft:2}}/>:null}</button>{palAdvanced&&<><div style={{marginTop:6}}><SliderRow label="Remove Orphans" value={orphans} min={0} max={3} onChange={setOrphans} format={v=>v===0?"Off":v}/></div><div style={{display:"flex",gap:6,marginTop:6}}><div style={{ display: "flex", gap: 2, background: "#f4f4f5", borderRadius: 8, padding: 2, flex: 1 }}><button onClick={()=>setDith(false)} style={{ padding: "5px 12px", fontSize: 12, fontWeight: !dith ? 500 : 400, background: !dith ? "#fff" : "transparent", borderRadius: 6, color: !dith ? "#18181b" : "#71717a", border: "none", cursor: "pointer", boxShadow: !dith ? "0 1px 2px rgba(0,0,0,0.04)" : "none", flex: 1 }}>Direct</button><button onClick={()=>setDith(true)} style={{ padding: "5px 12px", fontSize: 12, fontWeight: dith ? 500 : 400, background: dith ? "#fff" : "transparent", borderRadius: 6, color: dith ? "#18181b" : "#71717a", border: "none", cursor: "pointer", boxShadow: dith ? "0 1px 2px rgba(0,0,0,0.04)" : "none", flex: 1 }}>Dithered</button></div></div></>}</Section>
      ...
      {pat&&pal&&<div>
        ...
        {tab==="project"&&<div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <Section title="Pattern Summary">
            ...
            {confettiData&&(()=>{const cd=confettiData.clean;const t=confettiTier(cd.pct);const barW=Math.max(4,Math.round(100-cd.pct*6));return(
              <div style={{marginTop:8,padding:"8px 12px",background:"#fafafa",borderRadius:8,border:"0.5px solid #e4e4e7"}}>
                <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                  <div style={{fontSize:11,color:"#a1a1aa",textTransform:"uppercase",fontWeight:600,flexShrink:0}}>Stitchability</div>
                  <div style={{flex:1,minWidth:60,height:6,background:"#e4e4e7",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:barW+"%",background:t.color,borderRadius:3,transition:"width 0.4s"}}/></div>
                  <span style={{fontSize:12,fontWeight:700,color:t.color,flexShrink:0}}>{t.label}</span>
                  <span style={{fontSize:11,color:"#a1a1aa",flexShrink:0}}>{cd.singles.toLocaleString()} isolated ({cd.pct.toFixed(1)}%)</span>
                </div>
                {confettiData.raw.singles!==cd.singles&&<div style={{fontSize:10,color:"#a1a1aa",marginTop:4}}>{confettiData.raw.singles.toLocaleString()} before orphan removal</div>}
              </div>
            );})()}
      ...
      {!pat&&img&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(300px, 1fr))",gap:20}}>
        ...
        {previewUrl&&<div className="card" style={{display:"flex",flexDirection:"column"}}>
          ...
          {previewStats&&<div style={{padding:"10px 14px",borderTop:"0.5px solid #f4f4f5",background:"#fafafa",borderBottomLeftRadius:8,borderBottomRightRadius:8}}>
            ...
            {previewStats.confettiPct!=null&&(()=>{const t=confettiTier(previewStats.confettiPct);return(
              <div style={{marginTop:8,paddingTop:8,borderTop:"0.5px solid #e4e4e7"}}>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  <span style={{fontSize:10,color:"#a1a1aa",textTransform:"uppercase",fontWeight:600}}>Confetti</span>
                  <span style={{fontSize:12,fontWeight:700,color:t.color}}>{previewStats.confettiSingles.toLocaleString()} isolated ({previewStats.confettiPct.toFixed(1)}%)</span>
                  <span style={{fontSize:11,fontWeight:600,color:t.color,padding:"1px 6px",borderRadius:10,background:t.color+"18"}}>{t.label}</span>
                  {previewStats.confettiCleanSingles!=null&&previewStats.confettiCleanSingles<previewStats.confettiSingles&&<span style={{fontSize:10,color:"#a1a1aa"}}>→ {previewStats.confettiCleanSingles.toLocaleString()} after cleanup</span>}
                </div>
              </div>
            );})()}
          </div>}
