...
function saveProject(){
  ...
  let project={
    ...
    pattern:pat.map(m=>{if(m.id==="__skip__")return{id:"__skip__"};let o={id:m.id,type:m.type,rgb:m.rgb};if(m.stitchType&&m.stitchType!=="full")o.stitchType=m.stitchType;if(m.secondary)o.secondary=m.secondary;return o;}),
    bsLines,
...
function drawStitchShape(ctx, st, px, py, cSz, isDimmed, isDone, viewMode) {
  ...
  if (viewMode === "symbol") {
    ...
    fillAlpha = 1;
  } else if (viewMode === "color" || viewMode === "colour") { if (isDone) return; } else if (viewMode === "both") {
