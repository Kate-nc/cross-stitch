...
function ManagerApp() {
  ...
  const _UP = (k, fb) => {
    try {
      return window.UserPrefs && window.UserPrefs.get(k) || fb;
    } catch (_) {
      return fb;
    }
  };
  ...
  const [userProfile, setUserProfile] = useState(() => {
    const get = (k, fb) => {
      try {
        var v = window.UserPrefs && window.UserPrefs.get(k);
        return v == null ? fb : v;
      } catch (_) {
        return fb;
      }
  ...
  const [welcomeOpen, setWelcomeOpen] = useState(() => {
    try {
      return !!(window.WelcomeWizard && window.WelcomeWizard.shouldShow('manager'));
    } catch (_) {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    const onNarrow = () => {
      if (mql.matches) setPanelOpen(false);
    };
    if (mql.addEventListener) mql.addEventListener('change', onNarrow);else if (mql.addListener) mql.addListener(onNarrow);
    return () => {
      if (mql.removeEventListener) mql.removeEventListener('change', onNarrow);else if (mql.removeListener) mql.removeListener(onNarrow);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    return () => {
      if (window.CommandPalette) window.CommandPalette.registerPage('manager', []);
    };
  }, []);
  ...
  useEffect(() => {
    const h = e => {
      if (!e || !e.detail || e.detail.page === "manager") setWelcomeOpen(true);
    };
  ...
  const lowStockThreshold = (() => {
    try {
      var v = window.UserPrefs && window.UserPrefs.get("stashLowStockThreshold");
  ...
  const reconcileAutoSyncedPatterns = useCallback(async (basePatterns, allMeta) => {
    ...
    if (basePatterns.some(p => p.linkedProjectId && !metaIds.has(p.linkedProjectId))) {
      ...
      for (const p of basePatterns) {
        if (!p.linkedProjectId || metaIds.has(p.linkedProjectId)) {
          cleaned.push(p);
          continue;
        }
    ...
    const linkedIdxMap = new Map(cleaned.map((p, i) => p.linkedProjectId ? [p.linkedProjectId, i] : null).filter(Boolean));
  ...
  const initialLoadDoneRef = React.useRef({
    threads: false,
    patterns: false,
    profile: false
  });
  ...
  useEffect(() => {
    ...
    const loadManagerData = async () => {
      try {
        if (typeof window.loadAnchorData === 'function') {
          await window.loadAnchorData({
            failMessage: 'Could not load the Anchor catalogue for the Stash Manager. Reload and try again.'
          });
        }
        ...
        if (threadsData && versionData !== 2 && versionData !== 3) {
          ...
          for (const [id, t] of Object.entries(threadsData)) {
            finalThreads[id] = {
              ...t,
              partialStatus: t.partialStatus || null,
              min_stock: 0
            };
          }
          ...
          finalThreads = {};
          DMC.forEach(d => {
            finalThreads[threadKey('dmc', d.id)] = {
              owned: 0,
              tobuy: false,
              partialStatus: null,
              min_stock: 0
          ...
          if (typeof ANCHOR !== 'undefined') {
            ANCHOR.forEach(a => {
              finalThreads[threadKey('anchor', a.id)] = {
                owned: 0,
                tobuy: false,
                partialStatus: null,
                min_stock: 0
        ...
        if (threadsData && versionData === 3) {
          ...
          if (typeof ANCHOR !== 'undefined') {
            ANCHOR.forEach(a => {
              ...
              if (!migrated[aKey]) migrated[aKey] = {
                owned: 0,
                tobuy: false,
                partialStatus: null,
                min_stock: 0
    ...
    const handleStashChanged = async () => {
      try {
        ...
        console.warn('cs:stashChanged reload failed:', e);
      }
    };
  ...
  useEffect(() => {
    if (!initialLoadDoneRef.current.threads) {
      initialLoadDoneRef.current.threads = true;
      return;
    }
    const saveTimer = setTimeout(async () => {
      try {
        ...
        tx.oncomplete = () => {
          try {
            window.dispatchEvent(new CustomEvent('cs:stashChanged'));
          } catch (_) {}
        };
      } catch (err) {
        console.error("Threads auto-save failed:", err);
      }
    }, 1000);
    return () => clearTimeout(saveTimer);
  }, [threads]);
  useEffect(() => {
    if (!initialLoadDoneRef.current.patterns) {
      initialLoadDoneRef.current.patterns = true;
      return;
    }
    const saveTimer = setTimeout(async () => {
      try {
        ...
        tx.oncomplete = () => {
          try {
            window.dispatchEvent(new CustomEvent('cs:patternsChanged'));
          } catch (_) {}
        };
      } catch (err) {
        console.error("Patterns auto-save failed:", err);
      }
    }, 1000);
    return () => clearTimeout(saveTimer);
  }, [patterns]);
  useEffect(() => {
    if (!initialLoadDoneRef.current.profile) {
      initialLoadDoneRef.current.profile = true;
      return;
    }
    const saveTimer = setTimeout(async () => {
      try {
  ...
  useEffect(() => {
    ...
    const alerts = Object.entries(threads).filter(([, t]) => t.owned && t.owned > 0).filter(([compositeKey, t]) => isThreadLowStock(t, lowStockThreshold)).map(([compositeKey, t]) => {
      const {
        brand,
        bareId
      } = extractBrandAndId(compositeKey);
  ...
  function extractBrandAndId(compositeKey) {
    if (compositeKey.indexOf(':') < 0) return {
      brand: 'dmc',
      bareId: compositeKey
    };
  ...
  const handleRestoreFile = e => {
    ...
    reader.onload = () => {
      try {
        ...
        setBackupStatus({
          type: "error",
          message: "Invalid file: could not parse JSON."
        });
      }
    };
  ...
  const updateThread = (id, field, value) => {
    setThreads(prev => {
      ...
      if (prevEntry.addedAt !== undefined) {
        ...
        const newOwned = field === 'owned' ? value : field === 'partialStatus' && value === 'used-up' ? 0 : prevOwned;
        ...
        if (delta !== 0) {
          ...
          const newHistory = [...(prevEntry.history || []), {
            date: now,
            delta
          }];
  ...
  const filteredThreads = useMemo(() => {
    const dmcItems = DMC.map(d => ({
      ...d,
      brand: 'dmc',
      compositeKey: threadKey('dmc', d.id)
    }));
    const anchorItems = typeof ANCHOR !== 'undefined' ? ANCHOR.map(a => ({
      ...a,
      brand: 'anchor',
      compositeKey: threadKey('anchor', a.id)
    })) : [];
    const allItems = brandFilter === 'dmc' ? dmcItems : brandFilter === 'anchor' ? anchorItems : [...dmcItems, ...anchorItems];
    ...
    const filtered = searched.filter(d => {
      ...
      const t = threads[d.compositeKey] || {
        owned: 0,
        tobuy: false,
        partialStatus: null
      };
  ...
  const filteredPatterns = useMemo(() => {
    const q = searchQuery.toLowerCase();
    return patterns.filter(p => matchesSearch(p, q)).filter(p => matchesPatternFilter(p, patternFilter)).sort((a, b) => comparePatterns(a, b, patternSort));
  }, [patterns, searchQuery, patternFilter, patternSort]);
  function matchesSearch(p, q) {
    if (!q) return true;
    return p.title.toLowerCase().includes(q) || p.designer && p.designer.toLowerCase().includes(q) || p.tags && p.tags.some(tag => tag.toLowerCase().includes(q));
  }
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Header, {
    ...
    onOpenProject: typeof window.ProjectStorage !== 'undefined' ? () => {
    ...
    style: {
      ...
      borderRadius: 8,
      fontSize: 12,
      background: backupStatus.type === "error" ? "var(--danger-soft)" : backupStatus.type === "confirm" ? "var(--warning-soft)" : "var(--success-soft)",
      border: `1px solid ${backupStatus.type === "error" ? "var(--danger-soft)" : backupStatus.type === "confirm" ? "var(--warning)" : "#C4DCB6"}`,
      color: backupStatus.type === "error" ? "var(--danger)" : backupStatus.type === "confirm" ? "var(--accent-ink)" : "var(--success)"
    }
  }, /*#__PURE__*/React.createElement("div", null, backupStatus.message), backupStatus.summary && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      marginTop: 4,
      color: "var(--text-secondary)"
    }
  }, backupStatus.summary.projectCount, " projects, ", backupStatus.summary.threadCount, " owned threads, ", backupStatus.summary.patternCount, " patterns"), backupStatus.type === "confirm" && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 8,
      marginTop: 8
    ...
    onClick: backupStatus.onConfirm,
    style: {
      padding: "5px 12px",
      fontSize: 12,
      fontWeight: 600,
  ...
  }, "Yes, Restore"), /*#__PURE__*/React.createElement("button", {
    ...
    "data-onboard": "mgr-stash-tab",
    className: "mgr-tab" + (tab === "inventory" ? " on" : ""),
    onClick: () => {
      setTab("inventory");
      setSearchQuery("");
    ...
    className: "icon"
  }, Icons.thread()), " Thread Stash ", /*#__PURE__*/React.createElement("span", {
    className: "cnt"
  }, totalOwnedCount)), /*#__PURE__*/React.createElement("button", {
    "data-onboard": "mgr-patterns-tab",
    className: "mgr-tab" + (tab === "patterns" ? " on" : ""),
    onClick: () => {
      setTab("patterns");
      setSearchQuery("");
      setSelectedThread(null);
    ...
    className: "icon"
  }, Icons.clipboard()), " Pattern Library ", /*#__PURE__*/React.createElement("span", {
    className: "cnt"
  }, patterns.length))), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      window.location.href = "index.html?mode=stats&tab=showcase&from=home";
    },
  ...
  }, Icons.sparkles(), " Showcase"))), tab === "inventory" && /*#__PURE__*/React.createElement("div", {
    className: "mgr-filter-bar"
    ...
    key: f.id,
    className: "mgr-chip" + (threadFilter === f.id ? " on" : ""),
    ...
    key: 'brand-' + f.id,
    className: "mgr-chip" + (brandFilter === f.id ? " on" : ""),
    ...
    value: "colour"
  }, "Colour"), /*#__PURE__*/React.createElement("option", {
    value: "name"
  }, "Name (A-Z)"), /*#__PURE__*/React.createElement("option", {
    value: "owned_desc"
  }, "Owned: most first"), /*#__PURE__*/React.createElement("option", {
    value: "owned_asc"
  }, "Owned: fewest first")), typeof window.BulkAddModal !== 'undefined' && /*#__PURE__*/React.createElement("button", {
  ...
  }, "+ Bulk Add")), tab === "inventory" && /*#__PURE__*/React.createElement("div", {
    className: "mgr-stats-strip"
  }, /*#__PURE__*/React.createElement("div", {
    className: "app-info-chip-wrap mgr-stash-chip-wrap"
  }, /*#__PURE__*/React.createElement("button", {
    ref: stashChipRef,
    type: "button",
    ...
    className: "mgr-stash-chip-summary"
  }, totalOwnedCount, " skeins", lowStockNeeded && lowStockNeeded.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, " \u00B7 ", lowStockNeeded.length, " low")), /*#__PURE__*/React.createElement("span", {
    className: "app-info-chip__chevron",
    "aria-hidden": "true"
  }, Icons.chevronDown ? Icons.chevronDown() : null)), stashChipOpen && window.AppInfoPopover && (() => {
    ...
    const inventoryRows = [['Total skeins owned', totalOwnedCount.toLocaleString('en-GB')], ['Distinct threads owned', distinctOwned.toLocaleString('en-GB')], ['Low-stock threshold', String(lowStockThreshold)]];
    const statusRows = [];
    ...
    const badges = [];
    if (conflictCount > 0) badges.push({
      label: conflictCount + ' conflicts',
      kind: 'danger'
    });
    if (lowNeeded > 0) badges.push({
      label: lowNeeded + ' low',
      kind: 'warning'
    });
    if (readyCount > 0) badges.push({
      label: readyCount + ' ready',
      kind: 'success'
    });
    const children = [React.createElement(window.AppInfoSection, {
      key: 's-inv',
      title: 'Stash'
      ...
      key: 'd1'
    }), React.createElement(window.AppInfoSection, {
      key: 's-status',
      title: 'Status'
    }, React.createElement(window.AppInfoGrid, {
      rows: statusRows
    ...
    if (badges.length) {
      children.push(React.createElement(window.AppInfoDivider, {
        key: 'd2'
      }));
      children.push(React.createElement(window.AppInfoBadges, {
        key: 'b',
        items: badges
      }));
    }
  ...
  })())), tab === "inventory" && /*#__PURE__*/React.createElement("div", {
    className: "mgr-main"
  }, /*#__PURE__*/React.createElement("div", {
    className: "mgr-content"
  }, conflicts && conflicts.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "alert-card danger"
  }, /*#__PURE__*/React.createElement("div", {
    className: "at"
  }, Icons.warning(), " Thread Conflicts (", conflicts.length, ")"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-secondary)",
      marginBottom: 10
    }
  }, "These threads are needed by multiple patterns but you don't have enough."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
  ...
  }, conflicts.map(c => /*#__PURE__*/React.createElement("div", {
    key: c.id,
    style: {
      display: "flex",
      alignItems: "center",
  ...
  }, formatBrandLabel(c.brand), " ", c.id), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      color: "var(--text-secondary)",
      flex: 1
  ...
  }, "own ", c.owned, ", need ", c.totalNeeded), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      color: "var(--text-tertiary)"
    }
  }, c.patterns.map(p => p.title).join(", ")))))), lowStockNeeded && lowStockNeeded.length > 0 && /*#__PURE__*/React.createElement("div", {
    ...
    className: "at"
  }, Icons.box(), " Low Stock \u2014 Needed (", lowStockNeeded.length, ")"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-secondary)",
      marginBottom: 10
    }
  }, "Threads below your minimum stock level that are used by active projects."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 4,
  ...
  }, lowStockNeeded.map(a => /*#__PURE__*/React.createElement("div", {
    key: a.id,
    style: {
      display: "flex",
      alignItems: "center",
    ...
    style: {
      ...
      height: 14,
      borderRadius: 3,
      background: `rgb(${a.rgb[0]},${a.rgb[1]},${a.rgb[2]})`,
      border: "1px solid var(--border)",
      flexShrink: 0
    }
  }), /*#__PURE__*/React.createElement("span", {
  ...
  }, formatBrandLabel(a.brand), " ", a.bareId || a.id), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      color: "var(--text-secondary)",
      flex: 1
  ...
  }, "have ", a.owned, ", min ", a.min_stock))))), lowStockNotNeeded && lowStockNotNeeded.length > 0 && /*#__PURE__*/React.createElement("div", {
  ...
  }, Icons.box(), " Low stash \u2014 not currently needed (", lowStockNotNeeded.length, ")"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-tertiary)",
      marginBottom: 10
    }
  }, "These threads are below minimum stock but aren't used by any active project."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 4,
  ...
  }, lowStockNotNeeded.map(a => /*#__PURE__*/React.createElement("div", {
    key: a.id,
    style: {
      display: "flex",
      alignItems: "center",
  ...
  }, "have ", a.owned, ", min ", a.min_stock))))), /*#__PURE__*/React.createElement("div", {
    className: "thread-grid"
  }, filteredThreads.map(d => {
    const state = threads[d.compositeKey] || {
      owned: 0,
      tobuy: false,
      partialStatus: null
    };
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      onClick: () => {
        const next = isSelected ? null : d.compositeKey;
      ...
      className: "sw",
      style: {
        background: `rgb(${d.rgb})`
      }
    }), /*#__PURE__*/React.createElement("div", {
      className: "info"
    }, /*#__PURE__*/React.createElement("div", {
      className: "tid"
    }, d.id, d.brand === 'anchor' && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 9,
        fontWeight: 700,
        background: '#e0f2fe',
      ...
      className: "tnm"
    }, d.name)), isLowStock && /*#__PURE__*/React.createElement("span", {
      className: "badge-low"
  ...
  }, threadFilter === 'remnants' ? "Threads marked as remnants will appear here. You can change a thread's status from its entry in the All tab." : threadFilter === 'usedup' ? "Threads marked as used up will appear here." : "No threads found.")))), panelOpen && /*#__PURE__*/React.createElement("div", {
    className: "rpanel-backdrop",
    onClick: () => setPanelOpen(false)
  }), /*#__PURE__*/React.createElement("div", {
    className: "mgr-rpanel" + (panelOpen ? " mgr-rpanel--open" : "")
  }, /*#__PURE__*/React.createElement("div", {
    className: "mgr-panel-handle",
    onClick: () => setPanelOpen(o => !o)
  }, /*#__PURE__*/React.createElement("div", {
    className: "rpanel-handle-bar"
  }), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      color: "var(--text-tertiary)",
  ...
  }, selectedThread ? "Thread Detail" : "Thread Detail")), selectedThread ? (() => {
    ...
    const state = threads[selectedThread] || {
      owned: 0,
      tobuy: false,
      partialStatus: null,
      min_stock: 0
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      ...
      className: "dmc"
    }, brandLabel, " ", d.id, selBrand === 'anchor' && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 9,
        fontWeight: 700,
        background: '#e0f2fe',
      ...
      className: "lbl"
    }, "Full skeins"), /*#__PURE__*/React.createElement("div", {
      className: "qty-ctrl"
    }, /*#__PURE__*/React.createElement("button", {
      onClick: () => updateThread(selectedThread, "owned", Math.max(0, state.owned - 1))
    }, "-"), /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, state.owned), /*#__PURE__*/React.createElement("button", {
      onClick: () => updateThread(selectedThread, "owned", state.owned + 1)
    }, "+"))), /*#__PURE__*/React.createElement("div", {
      className: "td-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "lbl"
    }, "Min stock"), /*#__PURE__*/React.createElement("div", {
      className: "qty-ctrl"
    }, /*#__PURE__*/React.createElement("button", {
      onClick: () => updateThread(selectedThread, "min_stock", Math.max(0, (state.min_stock || 0) - 1))
    }, "-"), /*#__PURE__*/React.createElement("span", {
      className: "num"
    }, state.min_stock || 0), /*#__PURE__*/React.createElement("button", {
      onClick: () => updateThread(selectedThread, "min_stock", (state.min_stock || 0) + 1)
    }, "+"))), /*#__PURE__*/React.createElement("div", {
      className: "td-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "lbl"
    }, "Opened skein"), /*#__PURE__*/React.createElement("span", {
      className: "val",
      style: {
        fontSize: 11
      }
    }, !state.partialStatus ? "None" : state.partialStatus === "mostly-full" ? "Mostly full" : state.partialStatus === "about-half" ? "About half" : state.partialStatus === "remnant" ? "Remnant" : "Used up")), /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 8
      }
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "Opened skein level"), /*#__PURE__*/React.createElement("div", {
      className: "gauge-lg"
    }, [{
      val: null,
      label: "0"
      ...
      return /*#__PURE__*/React.createElement("div", {
        key: opt.val || "none",
        className: "seg" + (isActive ? " full" : ""),
        title: opt.val || "None",
        onClick: () => updateThread(selectedThread, "partialStatus", opt.val)
      }, opt.label);
    })))), /*#__PURE__*/React.createElement("div", {
      className: "rp-s"
    }, /*#__PURE__*/React.createElement("div", {
      ...
      className: "used-in"
    }, patternsUsingThread(d.id).length > 0 ? patternsUsingThread(d.id).map(p => /*#__PURE__*/React.createElement("div", {
      key: p.id || p.title,
      className: "ui-row"
    }, Icons.clipboard(), " ", p.title, " ", /*#__PURE__*/React.createElement("span", {
      className: "need"
    }, p.threads.find(t => t.id === d.id) ? `need ${p.threads.find(t => t.id === d.id).qty} sk` : ""))) : /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: "var(--text-tertiary)",
        padding: "4px 6px"
      }
    }, "Not used in any patterns"))), /*#__PURE__*/React.createElement("div", {
      className: "rp-s"
    }, /*#__PURE__*/React.createElement("div", {
      className: "rp-h"
  ...
  }, "Click a thread to view details"))), tab === "patterns" && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "mgr-filter-bar"
  }, /*#__PURE__*/React.createElement("input", {
    type: "search",
    ...
    key: f.id,
    className: "mgr-chip" + (patternFilter === f.id ? " on" : ""),
    ...
    value: "date_desc"
  }, "Sort: Recent"), /*#__PURE__*/React.createElement("option", {
    value: "date_asc"
  }, "Sort: Oldest"), /*#__PURE__*/React.createElement("option", {
    value: "title_asc"
  }, "Sort: Name"), /*#__PURE__*/React.createElement("option", {
    value: "designer_asc"
  }, "Sort: Designer"), /*#__PURE__*/React.createElement("option", {
    value: "status"
  }, "Sort: Status"))), /*#__PURE__*/React.createElement("div", {
    className: "mgr-stats-strip"
  }, /*#__PURE__*/React.createElement("div", {
    className: "stat"
  }, Icons.clipboard(), " ", /*#__PURE__*/React.createElement("span", {
    className: "val"
  }, patterns.length), " patterns"), /*#__PURE__*/React.createElement("div", {
    className: "stat"
  }, Icons.check(), " ", /*#__PURE__*/React.createElement("span", {
    className: "val"
  }, readyToStart ? readyToStart.filter(r => r.pct === 100).length : 0), " fully kitted")), /*#__PURE__*/React.createElement("div", {
    className: "mgr-main"
  }, /*#__PURE__*/React.createElement("div", {
    className: "mgr-content"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, Icons.gear(), " Thread Settings"), /*#__PURE__*/React.createElement("button", {
  ...
  }, "+ Add Pattern")), window.ProjectLibrary && /*#__PURE__*/React.createElement("div", {
    className: "mgr-project-library",
    style: {
      marginBottom: 16,
      padding: 12,
      border: "1px solid var(--border)",
  ...
  }, "Linked Creator/Tracker projects + Stash Manager-only entries")), React.createElement(window.ProjectLibrary, {
    mode: "manager",
    onOpenProject: (proj, target) => {
      if (!proj || !proj.id || proj.managerOnly) return;
      try {
        ProjectStorage.setActiveProject(proj.id);
      } catch (e) {}
    ...
    onOpenGlobalStats: () => {
      window.location.href = "index.html?mode=stats&from=home";
    },
    ...
    cardExtras: proj => {
      ...
      const meta = pat.linkedProjectId ? storedProjects.find(s => s.id === pat.linkedProjectId) : null;
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        title: completed.toLocaleString('en-GB') + " of " + total.toLocaleString('en-GB') + " stitches"
      }, pct, "% stitched"), weekly && weeklyTotal > 0 && /*#__PURE__*/React.createElement("span", {
        ...
        return /*#__PURE__*/React.createElement("span", {
          ...
          style: {
            width: BAR_W,
            height: h,
            background: v > 0 ? "#B85C38" : "var(--border)",
            borderRadius: 1,
            display: "inline-block"
          }
        });
      })), reqThreads.length > 0 && (missing.length === 0 ? /*#__PURE__*/React.createElement("span", {
        style: {
          padding: "2px 8px",
          borderRadius: 12,
        ...
        title: "All required threads are in your stash"
      }, Icons.check(), " Fully kitted") : /*#__PURE__*/React.createElement("span", {
        onClick: e => {
          e.stopPropagation();
          setViewingPattern(pat);
          setPanelOpen(true);
        },
        style: {
          padding: "2px 8px",
      ...
      }, missing.length, " threads needed")), pat.linkedProjectId && /*#__PURE__*/React.createElement("button", {
        type: "button",
        onClick: e => {
          e.stopPropagation();
          try {
            ProjectStorage.setActiveProject(pat.linkedProjectId);
          } catch (_) {}
          try {
      ...
      }, Icons.download ? Icons.download() : null, " Export pattern"));
    }
    ...
    className: "at"
  }, Icons.dot(), " Currently Tracking"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      justifyContent: "space-between",
      alignItems: "center"
    }
  }, /*#__PURE__*/React.createElement("span", null, activeProject.name || (activeProject.pattern && activeProject.pattern.length > 0 ? "Active Project" : "Unnamed Project")), /*#__PURE__*/React.createElement("a", {
    href: "stitch.html",
    style: {
      color: "#065f46",
      fontWeight: 600,
  ...
  }, "Go to Tracker ", Icons.chevronRight()))), readyToStart && readyToStart.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      background: "var(--success-soft)",
  ...
  }, Icons.check(), " Ready to Start"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-secondary)",
      marginBottom: 10
    }
  }, "Patterns you can fully kit from your current stash."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 6,
  ...
  }, readyToStart.map(r => /*#__PURE__*/React.createElement("div", {
    key: r.id,
    style: {
      display: "flex",
      alignItems: "center",
      ...
      borderRadius: 8,
      background: "var(--surface)",
      border: "1px solid " + (r.pct === 100 ? "#C4DCB6" : "var(--border)")
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1
  ...
  }, r.title || "Untitled"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: "var(--text-secondary)",
      marginTop: 2
    }
  }, r.totalThreads, " threads, ", r.coveredThreads, " covered (", r.pct, "%)")), r.pct === 100 ? /*#__PURE__*/React.createElement("span", {
    style: {
      padding: "3px 10px",
      borderRadius: 12,
      fontSize: 11,
  ...
  }, "100% kitted") : /*#__PURE__*/React.createElement("span", {
    style: {
      padding: "3px 10px",
      borderRadius: 12,
      fontSize: 11,
  ...
  }, r.pct, "% \u2014 ", r.missing.length, " missing"))))), storedProjects.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      background: "#f8fafc",
      border: "1px solid var(--border)",
      borderRadius: 10,
  ...
  }, "Saved Cross-Stitch Projects (", storedProjects.length, ")")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 8
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        border: `1px solid ${isActive ? "#C4DCB6" : "var(--border)"}`,
        borderRadius: 8,
        padding: "10px 14px"
      }
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, p.dimensions.width, "x", p.dimensions.height, " - ", pct, "% done - ", p.source === "tracker" ? "Tracked" : "Created", " - ", p.updatedAt ? new Date(p.updatedAt).toLocaleDateString('en-GB') : "")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: 6,
        flexShrink: 0,
      ...
      onClick: () => {
        ...
        window.__navigatingAway = true;
        window.location.href = "create.html?from=home&source=manager&id=" + encodeURIComponent(p.id);
      },
      ...
      onClick: () => {
        ProjectStorage.setActiveProject(p.id);
        window.__navigatingAway = true;
        window.location.href = "stitch.html?from=home&source=manager&id=" + encodeURIComponent(p.id);
      },
      style: {
        padding: "5px 10px",
        fontSize: 12,
      ...
      onClick: async () => {
        ...
        try {
          fullProject = await ProjectStorage.get(p.id);
        } catch (err) {
          console.error("Capture before delete failed:", err);
          try {
            window.Toast && window.Toast.show && window.Toast.show({
              message: 'Could not capture project before delete \u2014 aborting.',
              type: 'error'
            });
          } catch (_) {}
          return;
  ...
  }))), filteredPatterns.length === 0 && (patterns.length === 0 && patternFilter === 'all' && window.EmptyState ? React.createElement(window.EmptyState, {
  ...
  }, patterns.length === 0 ? 'No patterns yet. Click "+ Add Pattern" to start your library, or generate one in the Pattern Creator.' : "No patterns match your filters."))), panelOpen && /*#__PURE__*/React.createElement("div", {
    className: "rpanel-backdrop",
    onClick: () => setPanelOpen(false)
  }), /*#__PURE__*/React.createElement("div", {
    className: "mgr-rpanel" + (panelOpen ? " mgr-rpanel--open" : "")
  ...
  }, viewingPattern ? "Pattern Detail" : "Pattern Detail")), viewingPattern ? (() => {
    const p = viewingPattern;
    const coverage = p.threads && p.threads.length > 0 ? Math.round(p.threads.filter(t => {
      const k = normaliseStashKey(t.id);
      return (threads[k] || {}).owned > 0;
    }).length / p.threads.length * 100) : 0;
    const missingThreads = p.threads ? p.threads.filter(t => {
      const k = normaliseStashKey(t.id);
      return !(threads[k] || {}).owned;
    }) : [];
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "rp-s"
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, p.title || "Untitled"), p.designer && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: "var(--text-tertiary)",
        marginBottom: 8
      }
    }, "by ", p.designer), p.tags && p.tags.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: 4,
        flexWrap: "wrap",
      ...
      className: "status " + (p.status || "wishlist"),
      style: {
        display: "inline-block"
      }
    }, statusColors[p.status] ? statusColors[p.status].label : p.status)), /*#__PURE__*/React.createElement("div", {
      className: "rp-s"
    }, /*#__PURE__*/React.createElement("div", {
      className: "rp-h"
    }, "Thread Coverage ", /*#__PURE__*/React.createElement("span", {
      className: "badge"
    ...
    }, p.threads ? p.threads.filter(t => {
      const k = normaliseStashKey(t.id);
      return (threads[k] || {}).owned > 0;
    }).length : 0, " of ", p.threads ? p.threads.length : 0, " threads in your stash. ", missingThreads.length, " missing.")), missingThreads.length > 0 && /*#__PURE__*/React.createElement("div", {
      className: "rp-s"
    }, /*#__PURE__*/React.createElement("div", {
      className: "rp-h"
    }, "Missing Threads"), /*#__PURE__*/React.createElement("div", {
      className: "used-in"
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        title: "Open thread card for DMC " + t.id,
        onClick: () => {
          setTab("inventory");
          setThreadFilter("all");
          setBrandFilter("all");
          setSearchQuery("");
          setSelectedThread(compositeKey);
        }
      }, /*#__PURE__*/React.createElement("div", {
        style: {
          width: 12,
          height: 12,
          borderRadius: 3,
          background: dmc ? `rgb(${dmc.rgb})` : "#ccc",
          border: "1px solid var(--border)"
        }
      }), t.id, " ", dmc ? dmc.name : "", " ", /*#__PURE__*/React.createElement("span", {
        className: "need"
      }, t.qty ? t.qty + " sk" : ""));
    })), typeof StashBridge !== 'undefined' && /*#__PURE__*/React.createElement("button", {
      className: "g-btn",
      style: {
        width: "100%",
        justifyContent: "center",
      ...
      onClick: () => {
        ...
        StashBridge.markManyToBuy(keys, true).then(changed => {
          if (window.Toast) {
            ...
            window.Toast.show({
              message: changed > 0 ? changed + ' thread' + (changed !== 1 ? 's' : '') + ' added to shopping list' + (alreadyPresent > 0 ? '; ' + alreadyPresent + ' already on your shopping list' : '') : 'Already on your shopping list',
              type: 'success'
            });
          }
        }).catch(() => {
    ...
    }, Icons.cart(), " Add ", missingThreads.length, " to shopping list")), /*#__PURE__*/React.createElement("div", {
      className: "rp-s"
    }, /*#__PURE__*/React.createElement("div", {
      className: "rp-h"
    }, "Actions"), /*#__PURE__*/React.createElement("div", {
    ...
    }, Icons.pencil(), " Edit Pattern"), p.linkedProjectId && /*#__PURE__*/React.createElement("button", {
      className: "g-btn",
      style: {
        width: "100%",
        justifyContent: "center",
      ...
      onClick: () => {
        try {
          ProjectStorage.setActiveProject(p.linkedProjectId);
        } catch (_) {}
        try {
          localStorage.setItem('cs_pref_creator.lastPage', JSON.stringify('materials'));
          localStorage.setItem('cs_pref_creator.materialsTab', JSON.stringify('output'));
    ...
    }, Icons.download ? Icons.download() : null, " Export pattern"), /*#__PURE__*/React.createElement("button", {
      className: "g-btn",
      style: {
        width: "100%",
        justifyContent: "center"
      ...
      onClick: () => {
        duplicatePattern(p.id);
      }
    }, Icons.copy(), " Duplicate"), /*#__PURE__*/React.createElement("button", {
      className: "g-btn",
      style: {
        width: "100%",
        justifyContent: "center",
  ...
  }, "Click a pattern to view details")))), editingPattern && /*#__PURE__*/React.createElement(PatternModal, {
    pattern: editingPattern,
    onSave: p => {
      addOrUpdatePattern(p);
      if (viewingPattern) setViewingPattern(p);
    },
    onClose: () => {
      setEditingPattern(null);
      if (viewingPattern) setViewingPattern(viewingPattern);
    },
    inventoryThreads: threads,
    userProfile: userProfile
  }), profileModalOpen && /*#__PURE__*/React.createElement(UserProfileModal, {
    profile: userProfile,
    onSave: p => {
      setUserProfile(p);
  ...
  }), bulkAddOpen && window.BulkAddModal && React.createElement(window.BulkAddModal, {
    ...
    onClose: () => setModal(null)
  }), welcomeOpen && window.WelcomeWizard && /*#__PURE__*/React.createElement(window.WelcomeWizard, {
    page: "manager",
    onClose: () => setWelcomeOpen(false)
  }), window.HelpHintBanner && /*#__PURE__*/React.createElement(window.HelpHintBanner, null), modal === "about" && /*#__PURE__*/React.createElement(SharedModals.About, {
    onClose: () => setModal(null)
  }));
}
function PatternModal({
  ...
  const [edited, setEdited] = useState({
    ...
    project_overrides: pattern.project_overrides || null
  });
  ...
  const handleAddThread = e => {
    ...
    if (match) {
      ...
      setEdited({
        ...edited,
        threads: [...edited.threads, newThread]
      });
      setThreadInput("");
      setThreadQty(1);
  ...
  const removeTag = tagToRemove => {
    setEdited({
      ...edited,
      tags: edited.tags.filter(t => t !== tagToRemove)
    });
  };
  const removeThread = indexToRemove => {
    setEdited({
      ...edited,
      threads: edited.threads.filter((_, i) => i !== indexToRemove)
    });
  };
  const updateThreadQty = (idx, newQty) => {
    setEdited({
      ...edited,
      threads: edited.threads.map((t, i) => i === idx ? {
        ...t,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    role: "dialog",
    "aria-modal": "true",
    "aria-labelledby": "pattern-edit-title",
    onClick: onClose,
    style: {
      zIndex: 1000
    }
    ...
    id: "pattern-edit-title",
    style: {
      margin: 0,
      fontSize: 18
    }
  }, pattern.title ? "Edit Pattern" : "Add Pattern"), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    "aria-label": "Close",
    style: {
      background: "none",
  ...
  }, Icons.x ? Icons.x() : "\u00D7")), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: 20,
      overflowY: "auto",
      flex: 1,
  ...
  }, "Designer"), /*#__PURE__*/React.createElement("input", {
    type: "text",
    "aria-label": "Pattern designer",
    value: edited.designer,
    onChange: e => setEdited({
      ...edited,
    ...
    value: "owned"
  }, "Owned"), /*#__PURE__*/React.createElement("option", {
    value: "inprogress"
  }, "In Progress"), /*#__PURE__*/React.createElement("option", {
    value: "completed"
  }, "Completed"))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: {
      display: "block",
      fontSize: 12,
      fontWeight: 600,
  ...
  }, "Fabric & Dimensions"), /*#__PURE__*/React.createElement("input", {
    type: "text",
    "aria-label": "Fabric and dimensions",
    value: edited.fabric,
    onChange: e => setEdited({
      ...edited,
    ...
    placeholder: "e.g. 14ct Aida, 100x100"
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      gridColumn: "1 / -1"
    }
  ...
  }, "Tags (Press Enter to add)"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexWrap: "wrap",
      gap: 6,
  ...
  }, edited.tags.map(tag => /*#__PURE__*/React.createElement("span", {
    key: tag,
    style: {
      padding: "4px 8px",
      background: "#EFE7D6",
  ...
  }, Icons.x ? Icons.x() : "\u00D7")))), /*#__PURE__*/React.createElement("input", {
    type: "text",
    "aria-label": "Add tag",
    value: tagInput,
  ...
  }, "Thread Requirements"), /*#__PURE__*/React.createElement("form", {
    ...
    required: true
  }), showAutocomplete && autocompleteResults.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute",
      top: "100%",
      left: 0,
  ...
  }, autocompleteResults.map(res => /*#__PURE__*/React.createElement("div", {
    key: res.id,
    onClick: () => {
      setThreadInput(res.id);
      setShowAutocomplete(false);
    },
    style: {
      padding: "8px 12px",
  ...
  }, res.id), /*#__PURE__*/React.createElement("span", {
    style: {
      color: "var(--text-secondary)"
    }
  }, res.name))))), /*#__PURE__*/React.createElement("select", {
    value: threadBrand,
  ...
  }, Object.keys(BRAND_SKEIN_LENGTH).map(b => /*#__PURE__*/React.createElement("option", {
    key: b,
    value: b
    ...
    type: "button",
    onClick: () => handleUnitToggle("stitches"),
    style: {
      padding: "6px 10px",
      fontSize: 12,
      borderRadius: 6,
      border: "none",
      background: threadUnit === "stitches" ? "var(--surface)" : "transparent",
      fontWeight: threadUnit === "stitches" ? 600 : 400,
      cursor: "pointer",
      boxShadow: threadUnit === "stitches" ? "0 1px 2px rgba(0,0,0,0.05)" : "none"
    }
  }, "Stitches"), /*#__PURE__*/React.createElement("button", {
    type: "button",
    onClick: () => handleUnitToggle("skeins"),
    style: {
      padding: "6px 10px",
      fontSize: 12,
      borderRadius: 6,
      border: "none",
      background: threadUnit === "skeins" ? "var(--surface)" : "transparent",
      fontWeight: threadUnit === "skeins" ? 600 : 400,
      cursor: "pointer",
      boxShadow: threadUnit === "skeins" ? "0 1px 2px rgba(0,0,0,0.05)" : "none"
    }
  }, "Skeins"))), threadQty === "" && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: "#A06F2D"
    }
  }, "Quantity cleared \u2014 please enter the value in ", threadUnit, "."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 8,
      alignItems: "center",
  ...
  }), "Blended Thread")), isBlended && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 8,
      position: "relative",
    ...
    onChange: e => {
      setBlendColorInput(e.target.value);
      setShowBlendAutocomplete(true);
    },
    ...
    required: true
  }), showBlendAutocomplete && blendAutocompleteResults.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute",
      top: "100%",
      left: 0,
  ...
  }, blendAutocompleteResults.map(res => /*#__PURE__*/React.createElement("div", {
    key: res.id,
    onClick: () => {
      setBlendColorInput(res.id);
      setShowBlendAutocomplete(false);
    },
    style: {
      padding: "8px 12px",
    ...
    value: "1:1"
  }, "1:1 Ratio"), /*#__PURE__*/React.createElement("option", {
    value: "2:1"
  }, "2:1 Ratio"), /*#__PURE__*/React.createElement("option", {
    value: "1:2"
  }, "1:2 Ratio"), /*#__PURE__*/React.createElement("option", {
    value: "3:1"
  }, "3:1 Ratio"), /*#__PURE__*/React.createElement("option", {
    value: "1:3"
  }, "1:3 Ratio"))), /*#__PURE__*/React.createElement("button", {
    type: "submit",
    style: {
      padding: "8px 16px",
      background: "#B85C38",
  ...
  }, "Add Thread")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 8,
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        height: 20,
        borderRadius: 4,
        background: info ? `rgb(${info.rgb})` : "#ccc",
        border: "1px solid var(--border)"
      }
    }), /*#__PURE__*/React.createElement("div", {
      style: {
    ...
    }, t.is_blended ? `${t.name || info?.name || ""} + ${t.blend_name || t.blend_id} [${t.blend_ratio ? t.blend_ratio.join(':') : '1:1'}]` : t.name || info?.name || ""), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: 4
    ...
    }, displayUnit)), /*#__PURE__*/React.createElement("button", {
  ...
  }), edited.threads.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-tertiary)",
      textAlign: "center",
  ...
  }, "No threads added yet.")))), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "16px 20px",
  ...
  }, "Start Tracking ", Icons.chevronRight()), /*#__PURE__*/React.createElement("button", {
...
function PatternDetailsModal({
  ...
  const missingThreadsCount = useMemo(() => {
    ...
    Object.entries(requiredColors).forEach(([id, qty]) => {
      const invState = inventoryThreads[id] || {
        owned: 0
      };
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, pattern.title || "Untitled"), pattern.designer && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: "var(--text-secondary)"
    }
  }, "by ", pattern.designer)), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    "aria-label": "Close",
    style: {
      background: "none",
    ...
    style: {
      ...
      fontSize: 11,
      fontWeight: 600,
      background: statusColors[pattern.status].bg,
      color: statusColors[pattern.status].text
    }
  }, statusColors[pattern.status].label), pattern.fabric && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 13,
      color: "var(--text-secondary)"
    }
  }, "- ", pattern.fabric)), pattern.tags && pattern.tags.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 4,
      flexWrap: "wrap"
    }
  }, pattern.tags.map(tag => /*#__PURE__*/React.createElement("span", {
    key: tag,
    style: {
      padding: "2px 6px",
      background: "#EFE7D6",
  ...
  }, "Thread Requirements (", pattern.threads ? pattern.threads.length : 0, ")"), missingThreadsCount > 0 ? /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      color: "#B85555",
      fontWeight: 600,
  ...
  }, "Missing ", missingThreadsCount, " threads") : /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      color: "#4F7D3F",
      fontWeight: 600,
  ...
  }, "Have all threads")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 8
    }
  }, derivedThreads.length === 0 ? /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-tertiary)",
      textAlign: "center",
  ...
  }, "No threads specified.") : derivedThreads.map((t, idx) => {
    let text = "";
    let subtext = "";
    if (t.is_blended) {
    ...
    }), t.is_blended && /*#__PURE__*/React.createElement("div", {
      style: {
        width: 16,
        height: 16,
        borderRadius: 4,
    ...
    }, text)), subtext && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12,
        color: "var(--text-secondary)",
        paddingLeft: 28
  ...
  }, "Edit Pattern"), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    style: {
      padding: "8px 16px",
      borderRadius: 8,
...
function UserProfileModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Default Thread Settings"), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    "aria-label": "Close",
    style: {
      background: "none",
  ...
  }, "These settings are used to estimate the number of skeins required for your patterns based on stitch counts. You can override these for individual projects."), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: {
      display: "block",
      fontSize: 12,
      fontWeight: 600,
    ...
    value: edited.fabric_count,
    onChange: e => setEdited({
      ...edited,
      fabric_count: parseInt(e.target.value)
    }),
    style: {
      width: "100%",
      padding: "8px 12px",
  ...
  }, [11, 14, 16, 18, 20, 22, 25, 28, 32].map(ct => /*#__PURE__*/React.createElement("option", {
    key: ct,
    value: ct
  }, ct, " count")))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: {
      display: "block",
      fontSize: 12,
      fontWeight: 600,
  ...
  }, "Strands Used"), /*#__PURE__*/React.createElement("select", {
    value: edited.strands_used,
    onChange: e => setEdited({
      ...edited,
      strands_used: parseInt(e.target.value)
    }),
    style: {
      width: "100%",
      padding: "8px 12px",
  ...
  }, [1, 2, 3, 4, 5, 6].map(st => /*#__PURE__*/React.createElement("option", {
    key: st,
    value: st
  }, st, " strand", st > 1 ? "s" : "")))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: {
      display: "block",
      fontSize: 12,
      fontWeight: 600,
  ...
  }, "Preferred Thread Brand"), /*#__PURE__*/React.createElement("select", {
    value: edited.thread_brand,
    onChange: e => setEdited({
      ...edited,
      thread_brand: e.target.value
    }),
    style: {
      width: "100%",
      padding: "8px 12px",
  ...
  }, "Waste Factor"), /*#__PURE__*/React.createElement("select", {
    value: edited.waste_factor,
    onChange: e => setEdited({
      ...edited,
      waste_factor: parseFloat(e.target.value)
    }),
    style: {
      width: "100%",
      padding: "8px 12px",
    ...
    value: 0.10
  }, "Low (10% - Efficient stitching, few mistakes)"), /*#__PURE__*/React.createElement("option", {
    value: 0.20
  }, "Average (20% - Normal amount of travelling/mistakes)"), /*#__PURE__*/React.createElement("option", {
    value: 0.30
  }, "High (30% - Lots of confetti stitches/parking)")))), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "16px 20px",
      borderTop: "1px solid var(--border)",
      display: "flex",
