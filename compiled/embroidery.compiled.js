...
const STITCHES = [{
  id: "satin",
  name: "Satin",
  desc: "Smooth parallel fill",
  color: "var(--accent)"
}, {
  id: "longshort",
  name: "Long & Short",
  desc: "Blended shading fill",
  ...
  id: "chainstitch",
  name: "Chain",
  desc: "Looped fill/outline",
  color: "var(--accent)"
}, {
  id: "seedstitch",
  name: "Seed",
  desc: "Random short stitches",
  color: "#f59e0b"
}, {
  id: "stemstitch",
  name: "Stem",
...
const EMB_COLORS_LAB = EMB_COLORS.map(c => {
  const [cr, cg, cb] = hex2rgb(c.h);
  return {
    ...c,
    lab: rgb2lab(cr, cg, cb)
  };
});
function closestDMC(r, g, b) {
  const [L, a, bv] = rgb2lab(r, g, b);
  let best = EMB_COLORS_LAB[0],
    bd = 1e9;
  for (const c of EMB_COLORS_LAB) {
    const d = (L - c.lab[0]) ** 2 + (a - c.lab[1]) ** 2 + (bv - c.lab[2]) ** 2;
    if (d < bd) {
      bd = d;
      best = c;
    }
...
function suggestStitch(a) {
  if (a < 500) return "frenchknot";
  if (a < 2500) return "satin";
  if (a < 8000) return "longshort";
  return "chainstitch";
}
function ptIn(pts, px, py) {
  let ins = false;
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let t = 0; t < segs; t++) {
      const f = t / segs,
        f2 = f * f,
        f3 = f2 * f;
      out.push([.5 * (2 * p1[0] + (-p0[0] + p2[0]) * f + (2 * p0[0] - 5 * p1[0] + 4 * p2[0] - p3[0]) * f2 + (-p0[0] + 3 * p1[0] - 3 * p2[0] + p3[0]) * f3), .5 * (2 * p1[1] + (-p0[1] + p2[1]) * f + (2 * p0[1] - 5 * p1[1] + 4 * p2[1] - p3[1]) * f2 + (-p0[1] + 3 * p1[1] - 3 * p2[1] + p3[1]) * f3)]);
    }
  }
...
function simplify(pts, tol) {
  ...
  for (let i = 1; i < pts.length; i++) {
    const [lx, ly] = o[o.length - 1];
    if ((pts[i][0] - lx) ** 2 + (pts[i][1] - ly) ** 2 > tol * tol) o.push(pts[i]);
  }
  return o;
}

// Rebuild smooth curve from control nodes
function buildCurve(nodes) {
  if (nodes.length < 3) return nodes.slice();
  return catmull(nodes, 6);
}
function kMeans(data, w, h, k, iters = 15) {
  if (!data || !data.length || k <= 0 || w <= 0 || h <= 0) return {
    labels: new Int32Array(Math.max(0, (w | 0) * (h | 0))).fill(-1),
    centroids: []
  };
  const N = w * h,
    valid = [];
  for (let i = 0; i < N; i++) if (data[i * 4 + 3] > 30) valid.push(i);
  if (!valid.length) return {
    labels: new Int32Array(N).fill(-1),
    centroids: []
  };
  const centroids = [];
  let first = valid[Math.floor(Math.random() * valid.length)];
  centroids.push([data[first * 4], data[first * 4 + 1], data[first * 4 + 2]]);
  for (let c = 1; c < k; c++) {
    const step = Math.max(1, Math.floor(valid.length / 2000));
    let tD = 0;
    const ds = [];
    for (let s = 0; s < valid.length; s += step) {
      const idx = valid[s] * 4;
      let mD = 1e9;
      for (const ct of centroids) {
        const d = (data[idx] - ct[0]) ** 2 + (data[idx + 1] - ct[1]) ** 2 + (data[idx + 2] - ct[2]) ** 2;
        if (d < mD) mD = d;
      }
      ds.push({
        i: valid[s],
    ...
    centroids.push([data[pick * 4], data[pick * 4 + 1], data[pick * 4 + 2]]);
  }
  const labels = new Int32Array(N).fill(-1); /* PERF (perf-1 #3): hoisted sums Float64Array, .fill(0) per iter instead of allocating k arrays */
  ...
  for (let iter = 0; iter < iters; iter++) {
    for (let i = 0; i < N; i++) {
      ...
      let best = 0,
        bd = 1e9;
      for (let c = 0; c < k; c++) {
        const d = (data[idx] - centroids[c][0]) ** 2 + (data[idx + 1] - centroids[c][1]) ** 2 + (data[idx + 2] - centroids[c][2]) ** 2;
        if (d < bd) {
          bd = d;
          best = c;
        }
    ...
    for (let i = 0; i < N; i++) {
      ...
      const idx = i * 4,
        base = labels[i] * 4;
      sums[base] += data[idx];
      sums[base + 1] += data[idx + 1];
      sums[base + 2] += data[idx + 2];
      sums[base + 3]++;
    }
    let conv = true;
    for (let c = 0; c < k; c++) {
      const base = c * 4,
        cnt = sums[base + 3];
      if (!cnt) continue;
      const nr = sums[base] / cnt,
        ng = sums[base + 1] / cnt,
        nb = sums[base + 2] / cnt;
      if (Math.abs(nr - centroids[c][0]) > .5) conv = false;
...
function extractBoundary(mask, w, h) {
  const b = [];
  for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
    if (!mask[y * w + x]) continue;
    if (x === 0 || !mask[y * w + x - 1] || x === w - 1 || !mask[y * w + x + 1] || y === 0 || !mask[(y - 1) * w + x] || y === h - 1 || !mask[(y + 1) * w + x]) b.push([x, y]);
  }
  return b;
}
function traceContour(mask, w, h) {
  ...
  let sx = -1,
    sy = -1;
  outer: for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) if (mask[y * w + x]) {
    sx = x;
    sy = y;
    break outer;
  }
...
function downsample(pts, n) {
  ...
  const al = [0];
  for (let i = 1; i < pts.length; i++) al.push(al[i - 1] + Math.sqrt((pts[i][0] - pts[i - 1][0]) ** 2 + (pts[i][1] - pts[i - 1][1]) ** 2));
  const tot = al[al.length - 1];
  if (tot < 1) return pts.slice(0, n);
  const step = tot / n;
  const out = [pts[0]];
  let nd = step;
  for (let i = 1; i < pts.length && out.length < n; i++) while (al[i] >= nd && out.length < n) {
    const sl = al[i] - al[i - 1],
      t = sl > 0 ? (nd - al[i - 1]) / sl : 0;
...
function cannyEdges(data, w, h) {
  ...
  for (let i = 0; i < N; i++) {
    if (nms[i] >= CANNY_THRESHOLD_HIGH) edge[i] = STRONG;else if (nms[i] >= CANNY_THRESHOLD_LOW) edge[i] = WEAK;
  }
...
function lab2rgb(L, a, b) {
  const fy = (L + 16) / 116,
    fx = a / 500 + fy,
    fz = fy - b / 200;
  const f = t => t > .20690 ? t * t * t : (t - 16 / 116) / 7.787;
  let X = f(fx) * .95047,
    Y = f(fy),
    Z = f(fz) * 1.08883;
  let R = X * 3.2406 + Y * -1.5372 + Z * -.4986,
    G = X * -.9689 + Y * 1.8758 + Z * .0415,
    Bv = X * .0557 + Y * -.2040 + Z * 1.0570;
  const gm = c => Math.max(0, Math.min(255, Math.round((c > .0031308 ? 1.055 * c ** (1 / 2.4) - .055 : 12.92 * c) * 255)));
  return [gm(R), gm(G), gm(Bv)];
}
function slicSegment(data, w, h, k, compactness, iters, edgeMask = null) {
  const N = w * h,
    labL = new Float32Array(N),
    labA = new Float32Array(N),
    labBv = new Float32Array(N);
  for (let i = 0; i < N; i++) {
    const idx = i * 4;
    if (data[idx + 3] <= 30) {
      labL[i] = -1;
    ...
    const [lv, av, bv] = rgb2lab(data[idx], data[idx + 1], data[idx + 2]);
    labL[i] = lv;
    labA[i] = av;
    labBv[i] = bv;
  }
  ...
  for (let gy = 0; gy < rows && centres.length < k; gy++) for (let gx = 0; gx < cols && centres.length < k; gx++) {
    let cx = Math.min(w - 1, Math.round((gx + .5) * S)),
      cy = Math.min(h - 1, Math.round((gy + .5) * S)),
      bestG = 1e9;
    for (let dy = -1; dy <= 1; dy++) for (let dx = -1; dx <= 1; dx++) {
      const nx = cx + dx,
        ny = cy + dy;
      ...
      const pi = ny * w + nx;
      if (labL[pi] < 0) continue;
      const g = (labL[pi + 1] - labL[pi - 1]) ** 2 + (labL[(ny + 1) * w + nx] - labL[(ny - 1) * w + nx]) ** 2;
      if (g < bestG) {
        bestG = g;
        cx = nx;
        cy = ny;
    ...
    const ci = cy * w + cx;
    if (labL[ci] < 0) continue;
    centres.push([labL[ci], labA[ci], labBv[ci], cx, cy]);
  }
  ...
  for (let iter = 0; iter < iters; iter++) {
    ...
    for (let ci = 0; ci < nC; ci++) {
      const [cL, cA, cB, cx, cy] = centres[ci],
        r = Math.ceil(2 * S) | 0;
      const x0 = Math.max(0, cx - r | 0),
        x1 = Math.min(w - 1, cx + r | 0),
        y0 = Math.max(0, cy - r | 0),
        y1 = Math.min(h - 1, cy + r | 0);
    ...
    const sL = new Float64Array(nC),
      sA = new Float64Array(nC),
      sB = new Float64Array(nC),
      sX = new Float64Array(nC),
      sY = new Float64Array(nC),
      cnt = new Int32Array(nC);
    for (let i = 0; i < N; i++) {
      const c = labels[i];
      if (c < 0) continue;
      sL[c] += labL[i];
    ...
    for (let ci = 0; ci < nC; ci++) {
      if (!cnt[ci]) continue;
      centres[ci] = [sL[ci] / cnt[ci], sA[ci] / cnt[ci], sB[ci] / cnt[ci], sX[ci] / cnt[ci], sY[ci] / cnt[ci]];
    }
  }
  ...
  const connected = new Uint8Array(N),
    reachQ = [];
  for (let ci = 0; ci < nC; ci++) {
    const cx = Math.round(centres[ci][3]),
      cy = Math.round(centres[ci][4]),
      seed = cy * w + cx;
    if (seed < 0 || seed >= N || labels[seed] !== ci) continue;
    connected[seed] = 1;
    reachQ.push(seed);
  }
  for (let head = 0; head < reachQ.length; head++) {
    const p = reachQ[head],
      px = p % w,
      py = p / w | 0;
    for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
      ...
      if (nx < 0 || nx >= w || ny < 0 || ny >= h) continue;
      const ni = ny * w + nx;
      if (!connected[ni] && labels[ni] === labels[p]) {
        connected[ni] = 1;
        reachQ.push(ni);
      }
    }
  ...
  const spreadQ = [];
  for (let i = 0; i < N; i++) {
    if (labels[i] >= 0 && !connected[i]) labels[i] = -2;else if (connected[i]) spreadQ.push(i);
  }
  for (let head = 0; head < spreadQ.length; head++) {
    const p = spreadQ[head],
      px = p % w,
      py = p / w | 0;
    for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
      const nx = px + dx,
...
function mergeSuperpixels(labels, labL, labA, labBv, pw, ph, nSP, targetK, origData, oCW, oCH, scaledData, edgeW = 0.3) {
  const PN = pw * ph,
    spL = new Float64Array(nSP),
    spA = new Float64Array(nSP),
    spBv = new Float64Array(nSP),
    spCnt = new Int32Array(nSP);
  ...
  for (let i = 0; i < PN; i++) {
    const l = labels[i];
    if (l < 0 || l >= nSP) continue;
    spL[l] += labL[i];
    spA[l] += labA[i];
    spBv[l] += labBv[i];
    spCnt[l]++;
  }
  for (let s = 0; s < nSP; s++) if (spCnt[s]) {
    spL[s] /= spCnt[s];
    spA[s] /= spCnt[s];
    spBv[s] /= spCnt[s];
  }
  // Union-find
  const parent = new Int32Array(nSP);
  for (let s = 0; s < nSP; s++) parent[s] = s;
  const find = x => {
    while (parent[x] !== x) {
  ...
  const clL = Array.from(spL),
    clA = Array.from(spA),
    clBv = Array.from(spBv),
    clCnt = Array.from(spCnt);
  ...
  for (let y = 0; y < ph; y++) for (let x = 0; x < pw; x++) {
    ...
    const addEdge = oi => {
      const lo = labels[oi];
      if (lo >= 0 && lo !== li) {
        const a = Math.min(li, lo),
          b = Math.max(li, lo),
          key = a * nSP + b;
        if (!adjRaw.has(key)) adjRaw.set(key, {
          a,
          b,
          eSum: 0,
          eCnt: 0
        ...
        if (eMag) {
          const e = adjRaw.get(key);
          e.eSum += eMag[i] + eMag[oi];
          e.eCnt += 2;
        }
      }
    };
    if (x < pw - 1) addEdge(i + 1);
    if (y < ph - 1) addEdge(i + pw);
  }
  ...
  for (const [, e] of adjRaw) {
    if (!spCnt[e.a] || !spCnt[e.b]) continue;
    const ep = e.eCnt > 0 ? e.eSum / e.eCnt * edgeW : 0;
    edgePenalty.set(edgeKey(e.a, e.b), ep);
  }
  // Build initial adjacency list
  const mergeCost = (a, b) => {
    const cd = (clL[a] - clL[b]) ** 2 + (clA[a] - clA[b]) ** 2 + (clBv[a] - clBv[b]) ** 2;
    const ep = edgePenalty.get(edgeKey(a, b)) || 0;
    return cd + ep;
  };
  let adjList = [];
  for (const [, e] of adjRaw) {
  ...
  let nClusters = 0;
  for (let s = 0; s < nSP; s++) if (spCnt[s]) nClusters++;
  ...
  while (nClusters > targetK && adjList.length) {
    ...
    for (let i = 0; i < adjList.length; i++) {
      if (find(adjList[i].a) !== find(adjList[i].b)) {
        fi = i;
        break;
      }
    }
    ...
    const {
      ...
      ra = find(a),
      rb = find(b),
      tc = clCnt[ra] + clCnt[rb];
    clL[ra] = (clL[ra] * clCnt[ra] + clL[rb] * clCnt[rb]) / tc;
    clA[ra] = (clA[ra] * clCnt[ra] + clA[rb] * clCnt[rb]) / tc;
    clBv[ra] = (clBv[ra] * clCnt[ra] + clBv[rb] * clCnt[rb]) / tc;
    clCnt[ra] = tc;
    parent[rb] = ra;
    nClusters--;
    ...
    for (let i = 0; i < adjList.length; i++) {
      ...
      if (fa === ra || fb === ra) {
        if (fa !== fb) {
          const nc = fa === ra ? fb : fa;
          newN.add(nc);
          // Merge edge penalties: take max of (ra,nc) and (rb,nc)
          const kNew = edgeKey(ra, nc),
            kOld = edgePenalty.get(edgeKey(rb, nc));
          if (kOld !== undefined) {
            const existing = edgePenalty.get(kNew) || 0;
            edgePenalty.set(kNew, Math.max(existing, kOld));
          }
        }
        e.d = Infinity;
      }
    }
    for (const nc of newN) {
      if (!clCnt[nc]) continue;
      adjList.push({
        d: mergeCost(ra, nc),
        a: ra,
        b: nc
  ...
  for (let pass = 0; pass < 2; pass++) {
    ...
    for (let i = 0; i < PN; i++) {
      ...
      for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
        const nx2 = px + dx,
          ny2 = py + dy;
        if (nx2 < 0 || nx2 >= pw || ny2 < 0 || ny2 >= ph) continue;
        const ni2 = ny2 * pw + nx2,
          rn = refined[ni2];
        if (rn < 0 || rn === rl) continue;
  ...
  for (let i = 0; i < PN; i++) {
    ...
    if (!clusterMap.has(r)) clusterMap.set(r, {
      ...
      sb: 0,
      cnt: 0,
      lab: [clL[r], clA[r], clBv[r]]
    });
    const cl = clusterMap.get(r);
    mergedLabels[i] = cl.id;
    const ox = Math.min(oCW - 1, Math.round(i % pw * invSc)),
      oy = Math.min(oCH - 1, Math.round((i / pw | 0) * invSc)),
      oidx = (oy * oCW + ox) * 4;
    if (origData[oidx + 3] > 30) {
      cl.sr += origData[oidx];
      cl.sg += origData[oidx + 1];
      cl.sb += origData[oidx + 2];
      cl.cnt++;
    }
  }
...
function sobelMag(data, w, h) {
  ...
  for (let y = 2; y < h - 2; y++) for (let x = 2; x < w - 2; x++) {
    const p = (dx, dy) => lum[(y + dy) * w + (x + dx)];
    const gx5 = -p(-2, -2) - 2 * p(-1, -2) + 2 * p(1, -2) + p(2, -2) - 4 * p(-2, -1) - 8 * p(-1, -1) + 8 * p(1, -1) + 4 * p(2, -1) - 6 * p(-2, 0) - 12 * p(-1, 0) + 12 * p(1, 0) + 6 * p(2, 0) - 4 * p(-2, 1) - 8 * p(-1, 1) + 8 * p(1, 1) + 4 * p(2, 1) - p(-2, 2) - 2 * p(-1, 2) + 2 * p(1, 2) + p(2, 2);
    const gy5 = -p(-2, -2) - 4 * p(-1, -2) - 6 * p(0, -2) - 4 * p(1, -2) - p(2, -2) - 2 * p(-2, -1) - 8 * p(-1, -1) - 12 * p(0, -1) - 8 * p(1, -1) - 2 * p(2, -1) + 2 * p(-2, 1) + 8 * p(-1, 1) + 12 * p(0, 1) + 8 * p(1, 1) + 2 * p(2, 1) + p(-2, 2) + 4 * p(-1, 2) + 6 * p(0, 2) + 4 * p(1, 2) + p(2, 2);
...
function sobelMagLAB(labL, labA, labB, w, h) {
  ...
  for (let y = 1; y < h - 1; y++) for (let x = 1; x < w - 1; x++) {
    const g = arr => {
      const tl = arr[(y - 1) * w + (x - 1)],
        tc = arr[(y - 1) * w + x],
        tr = arr[(y - 1) * w + (x + 1)];
      const ml = arr[y * w + (x - 1)],
        mr = arr[y * w + (x + 1)];
      const bl = arr[(y + 1) * w + (x - 1)],
        bc = arr[(y + 1) * w + x],
        br = arr[(y + 1) * w + (x + 1)];
...
function magicWandFill(imageData, seedX, seedY, tolerance, useEdgeSnap, edgeFactor, occupiedMask, edgeMask = null, smoothedData = null) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const [hv, sv, lv] = rgb2hsl(cmpData[idx], cmpData[idx + 1], cmpData[idx + 2]);
    pH[i] = hv;
    pS[i] = sv;
    pLv[i] = lv;
  }
  ...
  if (!sc) return new Uint8Array(N);
  sH /= sc;
  sS /= sc;
  sLs /= sc;
  // Low-saturation guard: grey pixels - ignore hue, compare by lightness only
  let wH = WAND_WEIGHT_HUE,
    wS = WAND_WEIGHT_SATURATION,
    wL2 = WAND_WEIGHT_LIGHTNESS;
  if (sS < LOW_SAT_THRESHOLD) {
    wH = 0;
    wL2 = 1.5;
  }
  ...
  if (WAND_DYNAMIC_TOLERANCE_ENABLED) {
    ...
    if (dists.length > 1) {
      ...
      if (stddev < VARIANCE_LOW_THRESHOLD) effTol *= 0.7;else if (stddev > VARIANCE_HIGH_THRESHOLD) effTol *= 1.3;
    }
  }
  ...
  while (head < tail) {
    ...
    for (const [ddx, ddy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
      ...
      if (nx < 0 || nx >= w || ny < 0 || ny >= h) continue;
      const ni = ny * w + nx;
      if (visited[ni] || occupiedMask[ni] || pH[ni] < 0) continue;
      // Hard Canny boundary: treat edge pixels as impassable walls
      if (edgeMask && edgeMask[ni]) {
        visited[ni] = 1;
        continue;
      }
      visited[ni] = 1;
      ...
      let et = effTol;
      if (useEdgeSnap && edgeMag) {
        const eStr = Math.min(1, edgeMag[ni] / 200);
        et = effTol / (1 + eStr * edgeFactor);
      }
...
function autoSegment(imageData, numColors, compactness = 10) {
  ...
  const PROC_MAX = 400,
    scale = Math.min(1, PROC_MAX / Math.max(CW, CH));
  const pw = Math.max(2, Math.round(CW * scale)),
    ph = Math.max(2, Math.round(CH * scale));
  ...
  const complexity = (() => {
    ...
    const eMn = eS / eN,
      eStd = Math.sqrt(Math.max(0, eS2 / eN - eMn * eMn));
    return Math.min(2, Math.max(0.5, (eMn + eStd) / (eMn + 1)));
  })();
  ...
  try {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const [l, a, b] = rgb2lab(D[idx], D[idx + 1], D[idx + 2]);
      fL[i] = l;
      fA[i] = a;
      fB[i] = b;
    }
    const fullSmoothed = bilateralFilter(D, CW, CH);
    _wandEdgeCache = {
      mag: sobelMagLAB(fL, fA, fB, CW, CH),
      canny: cannyEdges(fullSmoothed, CW, CH),
      smoothed: fullSmoothed,
      w: CW,
      h: CH
    };
    _lassoCostCache = null;
  ...
  const {
    ...
    labBv,
    nC
  } = slicSegment(smoothed, pw, ph, nSP, slicM, 10, edgeMask);
  const {
    mergedLabels,
    avgColors
  } = mergeSuperpixels(spLabels, labL, labA, labBv, pw, ph, nC, numColors, data, CW, CH, smoothed, edgeW);
  // Connected components on merged label map
  const PN = pw * ph,
    visited = new Uint8Array(PN),
    minSize = Math.max(6, Math.floor(PN / 400)),
    components = [];
  for (let i = 0; i < PN; i++) {
    if (visited[i] || mergedLabels[i] < 0) continue;
    const col = mergedLabels[i],
      stack = [i];
    visited[i] = 1;
    const pix = [];
    while (stack.length) {
      ...
      for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
        ...
        if (nx >= 0 && nx < pw && ny >= 0 && ny < ph) {
          const ni = ny * pw + nx;
          if (!visited[ni] && mergedLabels[ni] === col) {
            visited[ni] = 1;
            stack.push(ni);
          }
        }
    ...
    if (pix.length >= minSize) components.push({
      pixels: pix,
      col: avgColors[col] || [180, 180, 180],
      size: pix.length
    });
  }
  ...
  {
    const cLbl = new Int32Array(PN).fill(-1);
    for (let ci = 0; ci < components.length; ci++) for (const p of components[ci].pixels) cLbl[p] = ci;
    ...
    for (let i = 0; i < PN; i++) {
      ...
      for (const [dx, dy] of [[1, 0], [0, 1]]) {
        ...
        const cj = cLbl[ny * pw + nx];
        if (cj < 0 || cj === ci) continue;
        const ri = cF(ci),
          rj = cF(cj);
        if (ri !== rj && cD[ri] === cD[rj]) cP[rj] = ri;
      }
    }
    const mMap = new Map();
    for (let ci = 0; ci < components.length; ci++) {
      const r = cF(ci);
      if (!mMap.has(r)) mMap.set(r, {
        pixels: [],
        col: components[r].col,
        size: 0
      });
      const mc = mMap.get(r);
      for (const p of components[ci].pixels) mc.pixels.push(p);
      mc.size += components[ci].size;
    }
    for (const [, mc] of mMap) {
      let sr = 0,
        sg = 0,
        sb = 0,
        cnt = 0;
      for (const p of mc.pixels) {
        const ox = Math.min(CW - 1, Math.round(p % pw * invSc)),
          oy = Math.min(CH - 1, Math.round((p / pw | 0) * invSc)),
          oi = (oy * CW + ox) * 4;
        if (data[oi + 3] > 30) {
          sr += data[oi];
          sg += data[oi + 1];
      ...
      if (cnt > 0) mc.col = [Math.round(sr / cnt), Math.round(sg / cnt), Math.round(sb / cnt)];
    }
    components.length = 0;
    for (const [, mc] of mMap) if (mc.size >= minSize) components.push(mc);
    components.sort((a, b) => b.size - a.size);
  }
  ...
  for (const comp of components) {
    ...
    const bMask = new Uint8Array(PN);
    for (let y = 0; y < ph; y++) for (let x = 0; x < pw; x++) {
      if (!useMask[y * pw + x]) continue;
      if (x === 0 || !useMask[y * pw + x - 1] || x === pw - 1 || !useMask[y * pw + x + 1] || y === 0 || !useMask[(y - 1) * pw + x] || y === ph - 1 || !useMask[(y + 1) * pw + x]) bMask[y * pw + x] = 1;
    }
    ...
    const tmpB = new Uint8Array(bMask);
    let bestContour = [];
    for (let scan = 0; scan < PN; scan++) {
      if (!tmpB[scan]) continue;
      const c = traceContour(tmpB, pw, ph);
      for (const [cx, cy] of c) tmpB[cy * pw + cx] = 0;
    ...
    const snapped = bestContour.map(([cx, cy]) => {
      let bm = sMag[cy * pw + cx],
        bx = cx,
        by = cy;
      for (let dy = -2; dy <= 2; dy++) for (let dx = -2; dx <= 2; dx++) {
        const nx = cx + dx,
    ...
    const ds = downsample(contour, tp);
    if (ds.length < 4) continue;
    let sm = catmull(ds, 3);
    sm = simplify(sm, 1.5);
    sm = smoothContourAngles(sm);
    if (sm.length < 4) continue;
    const sc = sm.map(([x, y]) => [x * invSc, y * invSc]);
    const dmc = closestDMC(...comp.col),
      area = pArea(sc),
      bounds = pBounds(sc);
    ...
    regions.push({
      ...
      nodes,
      points: curve,
      bounds: pBounds(curve),
      avgColor: comp.col,
      dmc,
      stitch: suggestStitch(area),
      direction: 0,
      area: pArea(curve)
    });
  }
  return regions.slice(0, 20).map((r, i) => ({
    ...r,
    id: i + 1,
    label: `Region ${i + 1}`
  }));
}
function renderStitch(ctx, pts, b, type, dir) {
  ctx.save();
  ctx.beginPath();
  ...
  const cx = b.x + b.w / 2,
    cy = b.y + b.h / 2,
    rad = (dir || 0) * Math.PI / 180,
    cos = Math.cos(rad),
    sin = Math.sin(rad),
    dg = Math.sqrt(b.w ** 2 + b.h ** 2) + 20;
  if (type === "satin") {
    for (let i = -dg; i < dg; i += 3) {
      ctx.strokeStyle = i % 2 === 0 ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.12)";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(cx + i * cos - dg * sin, cy + i * sin + dg * cos);
      ctx.lineTo(cx + i * cos + dg * sin, cy + i * sin - dg * cos);
      ctx.stroke();
    }
  } else if (type === "longshort") {
    for (let i = -dg; i < dg; i += 5) {
      ctx.strokeStyle = i % 2 === 0 ? "rgba(255,255,255,0.18)" : "rgba(0,0,0,0.1)";
      ctx.lineWidth = 1;
      const len = dg * (.35 + Math.abs(i * 13 % 7) / 14);
      ctx.beginPath();
      ctx.moveTo(cx + i * cos - len * sin, cy + i * sin + len * cos);
      ctx.lineTo(cx + i * cos + len * sin, cy + i * sin - len * cos);
      ctx.stroke();
    }
  } else if (type === "frenchknot") {
    for (let px = b.x; px < b.x + b.w; px += 5) for (let py = b.y; py < b.y + b.h; py += 5) {
      if (!ptIn(pts, px, py)) continue;
      ctx.fillStyle = "rgba(0,0,0,0.15)";
      ctx.beginPath();
      ctx.arc(px + Math.sin(px * 7 + py * 11) * 2, py + Math.cos(px * 11 + py * 7) * 2, 1.8, 0, Math.PI * 2);
      ctx.fill();
    }
  } else if (type === "chainstitch") {
    ctx.lineWidth = .8;
    for (let i = -dg; i < dg; i += 12) for (let j = -dg; j < dg; j += 7) {
      const px = cx + i * cos + j * sin,
        py = cy + i * sin - j * cos;
      if (!ptIn(pts, px, py)) continue;
      ctx.strokeStyle = "rgba(0,0,0,0.15)";
      ctx.beginPath();
      ctx.ellipse(px, py, 2, 3.5, rad, 0, Math.PI * 2);
      ctx.stroke();
    }
  } else if (type === "seedstitch") {
    ctx.lineWidth = 1.2;
    for (let px = b.x; px < b.x + b.w; px += 6) for (let py = b.y; py < b.y + b.h; py += 6) {
      if (!ptIn(pts, px, py)) continue;
      const a = Math.sin(px * 11 + py * 7) * Math.PI,
        l = 1.5 + Math.abs(Math.cos(px * 3 + py * 9)) * 2;
      ctx.strokeStyle = "rgba(0,0,0,0.15)";
      ctx.beginPath();
  ...
  } else if (type === "stemstitch") {
    ctx.lineWidth = 1.8;
    ctx.setLineDash([5, 3]);
    ctx.strokeStyle = "rgba(0,0,0,0.2)";
    for (let i = -dg; i < dg; i += 8) {
      ctx.beginPath();
      ctx.moveTo(cx + i * cos - dg * sin, cy + i * sin + dg * cos);
      ctx.lineTo(cx + i * cos + dg * sin, cy + i * sin - dg * cos);
      ctx.stroke();
...
function renderArrow(ctx, b, dir) {
  const len = Math.min(b.w, b.h) * .2;
  if (len < 5) return;
  const cx = b.x + b.w / 2,
    cy = b.y + b.h / 2,
    rad = (dir || 0) * Math.PI / 180,
  ...
  ctx.beginPath();
  ctx.moveTo(cx + cos * len, cy + sin * len);
  ctx.lineTo(cx + cos * len - cos * a - sin * a * .5, cy + sin * len - sin * a + cos * a * .5);
  ctx.lineTo(cx + cos * len - cos * a + sin * a * .5, cy + sin * len - sin * a - cos * a * .5);
  ctx.closePath();
  ctx.fill();
  ctx.restore();
...
function getRecommendations(region, allRegions) {
  ...
  const aspect = bounds.w > 0 && bounds.h > 0 ? Math.max(bounds.w, bounds.h) / Math.min(bounds.w, bounds.h) : 1;
  const isNarrow = aspect > 4,
    isTiny = area < 300,
    isSmall = area < 800,
    isMedium = area >= 800 && area < 3000,
    isLarge = area >= 3000,
    isVLarge = area >= 6000;
  if (stitch === "satin" && isLarge) recs.push({
    type: "warning",
    icon: "warning",
    msg: "Region may be too large for satin — threads can sag.",
    suggest: "Try Long & Short for better coverage.",
    fix: {
      stitch: "longshort"
    }
  });
  if (stitch === "frenchknot" && isLarge) recs.push({
    type: "tip",
    icon: "lightbulb",
    msg: "Large area for French knots — very time-consuming.",
    suggest: "Seed stitch gives similar texture with less effort.",
    fix: {
      stitch: "seedstitch"
    }
  });
  if (stitch === "longshort" && isSmall) recs.push({
    type: "tip",
    icon: "lightbulb",
    msg: "Small enough for clean satin stitch — smoother finish.",
    suggest: "Switch to Satin.",
    fix: {
      stitch: "satin"
    }
  });
  if (stitch === "stemstitch" && isVLarge) recs.push({
    type: "warning",
    icon: "warning",
    msg: "Stem stitch is for outlines, not large fills.",
    suggest: "Use Chain stitch for a looped fill.",
    fix: {
      stitch: "chainstitch"
    }
  });
  if (isNarrow && (stitch === "longshort" || stitch === "chainstitch" || stitch === "seedstitch")) recs.push({
    type: "tip",
    icon: "lightbulb",
    msg: "Narrow shape — fill stitches may not read well.",
    suggest: "Stem stitch along the length works naturally.",
    fix: {
      stitch: "stemstitch",
      direction: bounds.w > bounds.h ? 0 : 90
    }
  });
  if (isTiny && (stitch === "longshort" || stitch === "chainstitch" || stitch === "seedstitch")) recs.push({
    type: "tip",
    icon: "lightbulb",
    msg: "Very small region — a few French knots may be enough.",
    suggest: "Switch to French Knots.",
    fix: {
      stitch: "frenchknot"
    }
  });
  if (stitch === "satin" && isNarrow) {
    const id = bounds.w > bounds.h ? 90 : 0;
    if (Math.abs(direction - id) > 20 && Math.abs(direction - id - 360) > 20) recs.push({
      type: "tip",
      icon: "lightbulb",
      msg: "Satin stitches look best across the narrow width.",
      suggest: `Try ${id}° for perpendicular stitches.`,
      fix: {
        direction: id
      }
    });
  }
  if (stitch === "satin" && !isNarrow && isMedium && direction === 0 && bounds.w !== bounds.h) recs.push({
    type: "tip",
    icon: "compass",
    msg: "Default direction may not suit this shape. Try adjusting for a natural flow.",
    suggest: "Rotate to follow the shape.",
    fix: null
  });
  for (const other of allRegions) {
    if (other.id === region.id) continue;
    const gap = 15;
    const near = !(other.bounds.x > bounds.x + bounds.w + gap || other.bounds.x + other.bounds.w < bounds.x - gap || other.bounds.y > bounds.y + bounds.h + gap || other.bounds.y + other.bounds.h < bounds.y - gap);
    if (!near) continue;
    const cd = (region.avgColor[0] - other.avgColor[0]) ** 2 + (region.avgColor[1] - other.avgColor[1]) ** 2 + (region.avgColor[2] - other.avgColor[2]) ** 2;
    if (cd < 3000 && cd > 200) {
      recs.push({
        type: "tip",
        icon: "palette",
        msg: `Similar colour to "${other.label}" nearby.`,
        suggest: "Long & Short can blend them at the boundary.",
        fix: {
          stitch: "longshort"
        }
      });
...
function fillPolygon(pts, w, h) {
  ...
  const yMin = Math.max(0, Math.floor(Math.min(...ys))),
    yMax = Math.min(h - 1, Math.ceil(Math.max(...ys)));
  const n = pts.length;
  for (let y = yMin; y <= yMax; y++) {
    ...
    for (let k = 0; k + 1 < xs.length; k += 2) {
      const x0 = Math.max(0, Math.ceil(xs[k])),
        x1 = Math.min(w - 1, Math.floor(xs[k + 1]));
...
function getLassoCostData(imageData) {
  if (_wandEdgeCache && _wandEdgeCache.canny && _wandEdgeCache.w === CW && _wandEdgeCache.h === CH) {
    ...
    const c = _wandEdgeCache.canny,
      d = new Uint8Array(CW * CH);
...
function lassoPathfind(costData, w, h, ax, ay, bx, by) {
  const dx0 = bx - ax,
    dy0 = by - ay,
    straightDist = Math.sqrt(dx0 * dx0 + dy0 * dy0);
  ...
  const cx0 = Math.max(0, Math.min(ax, bx) - R),
    cx1 = Math.min(w - 1, Math.max(ax, bx) + R);
  const cy0 = Math.max(0, Math.min(ay, by) - R),
    cy1 = Math.min(h - 1, Math.max(ay, by) + R);
  const N = w * h,
    distArr = new Float32Array(N).fill(Infinity),
    prevArr = new Int32Array(N).fill(-1);
  const start = ay * w + ax,
    goal = by * w + bx;
  distArr[start] = 0;
  // Inline min-heap
  ...
  const hpDown = k => {
    ...
    for (;;) {
      let s = k,
        l = 2 * k + 1,
        r = 2 * k + 2;
      if (l < n && hp[l].c < hp[s].c) s = l;
      if (r < n && hp[r].c < hp[s].c) s = r;
      if (s === k) break;
      [hp[s], hp[k]] = [hp[k], hp[s]];
      k = s;
    }
...
function EmbroideryApp() {
  ...
  const mainC = useRef(null),
    imgC = useRef(null),
    fileRef = useRef(null);
  const imgRef = useRef(null),
    imgDataRef = useRef(null),
    isDraw = useRef(false);
  ...
  const handleFile = useCallback(e => {
    ...
    reader.onload = ev => {
      ...
      img.onload = () => {
        ...
        imgDataRef.current = ctx.getImageData(0, 0, CW, CH);
        setPhase("segment");
      };
      img.onerror = () => {
        imgRef.current = null;
        ...
        try {
          window.alert("Could not decode image. The file may be corrupt or in an unsupported format.");
        } catch (_) {}
      };
    ...
    reader.onerror = () => {
      ...
      imgDataRef.current = null;
      try {
        window.alert("Could not read file: " + (reader.error && reader.error.message ? reader.error.message : "unknown error"));
      } catch (_) {}
    };
    reader.onabort = () => {
      imgRef.current = null;
  ...
  const runAuto = useCallback(() => {
    ...
    setLoading(true);
    setTimeout(() => {
      const r = autoSegment(imgDataRef.current, numColors, compactness);
      setRegions(r);
  ...
  const makeRegion = useCallback(rawPts => {
    ...
    let sm = simplify(rawPts, 4);
    if (sm.length >= 4) sm = catmull(sm, 4);
    sm = simplify(sm, 2);
    if (sm.length < 4) return null;
    const bounds = pBounds(sm),
      area = pArea(sm);
    let r = 180,
      g = 180,
      b = 180,
    ...
    if (imgDataRef.current) {
      ...
      for (let py = Math.max(0, bounds.y | 0); py < Math.min(CH, bounds.y + bounds.h); py += 2) for (let px = Math.max(0, bounds.x | 0); px < Math.min(CW, bounds.x + bounds.w); px += 2) if (ptIn(sm, px, py)) {
        const i = (py * CW + px) * 4;
        r += d[i];
        g += d[i + 1];
        b += d[i + 2];
    ...
    const nodes = pointsToNodes(sm, Math.min(16, Math.max(6, Math.floor(Math.sqrt(area) / 6))));
    const curve = buildCurve(nodes);
    return {
      nodes,
      points: curve,
  ...
  const finishDraw = useCallback(() => {
    const result = makeRegion(curPts);
    ...
    setNextId(n => n + 1);
  }, [curPts, nextId, makeRegion]);
  const runWand = useCallback((mx, my) => {
    if (!imgDataRef.current) return;
    const sx = Math.max(0, Math.min(CW - 1, Math.round(mx))),
      sy = Math.max(0, Math.min(CH - 1, Math.round(my)));
    // Build occupied mask from existing regions
    const occ = new Uint8Array(CW * CH);
    for (const r of regions) {
      const b = r.bounds;
      for (let py = Math.max(0, b.y | 0); py < Math.min(CH, b.y + b.h + 1 | 0); py++) for (let px = Math.max(0, b.x | 0); px < Math.min(CW, b.x + b.w + 1 | 0); px++) if (ptIn(r.points, px, py)) occ[py * CW + px] = 1;
    }
    ...
    for (let y = 0; y < CH; y++) for (let x = 0; x < CW; x++) {
      ...
      if (x === 0 || !mask[y * CW + x - 1] || x === CW - 1 || !mask[y * CW + x + 1] || y === 0 || !mask[(y - 1) * CW + x] || y === CH - 1 || !mask[(y + 1) * CW + x]) bMask[y * CW + x] = 1;
    }
    const contour = traceContour(bMask, CW, CH);
    if (contour.length < 10) return;
    const tp = Math.min(80, Math.max(16, Math.floor(Math.sqrt(contour.length) / 3)));
    const ds = downsample(contour, tp);
    if (ds.length < 4) return;
    let sm = catmull(ds, 3);
    sm = simplify(sm, 2);
    sm = smoothContourAngles(sm);
    if (sm.length < 4) return;
    let ar = 0,
      ag = 0,
      ab = 0,
      ac = 0;
  ...
  const finishLasso = useCallback(() => {
    ...
    let ar = 0,
      ...
      ab = 0,
      ac = 0;
    const d = imgDataRef.current ? imgDataRef.current.data : new Uint8Array(0);
    for (let i = 0; i < CW * CH; i++) {
      if (!mask[i]) continue;
      ar += d[i * 4];
      ag += d[i * 4 + 1];
  ...
  useEffect(() => {
    ...
    ctx.scale(z, z);
    try {
      if (imgRef.current && (view === "original" || view === "overlay")) {
        const img = imgRef.current,
          s = Math.min(CW / img.width, CH / img.height);
        ctx.globalAlpha = view === "overlay" ? .35 : 1;
        ctx.drawImage(img, (CW - img.width * s) / 2, (CH - img.height * s) / 2, img.width * s, img.height * s);
        ctx.globalAlpha = 1;
      }
      if (view !== "original") {
        for (const r of regions) {
          ...
          ctx.closePath();
          ctx.fillStyle = r.dmc.h + (view === "overlay" ? "88" : "cc");
          ctx.fill();
          ctx.restore();
          renderStitch(ctx, r.points, r.bounds, r.stitch, r.direction);
        }
        for (const r of regions) {
          ...
          for (let i = 1; i < r.points.length; i++) ctx.lineTo(r.points[i][0], r.points[i][1]);
          ctx.closePath();
          ctx.strokeStyle = isSel ? ACCENT : "rgba(255,255,255,0.7)";
          ctx.lineWidth = (isSel ? 3 : 1.5) / z;
          if (isSel) {
    ...
    // Rec badges
    if (view !== "original" && editMode === "select") {
      for (const r of regions) {
        const recs = getRecommendations(r, regions).filter(rc => !dismissed.has(`${r.id}-${rc.msg.slice(0, 20)}`));
        if (recs.length > 0) {
          const hw = recs.some(rc => rc.type === "warning");
          const [bx, by] = cb(r.bounds.x + r.bounds.w - 4, r.bounds.y + 6);
          ctx.save();
          ctx.beginPath();
          ctx.arc(bx, by, 5, 0, Math.PI * 2);
          ctx.fillStyle = hw ? "#f59e0b" : "#60a5fa";
          ctx.fill();
          ctx.strokeStyle = "#fff";
          ctx.lineWidth = 1.5;
          ctx.stroke();
          ...
          ctx.fillText(recs.length.toString(), bx, by + 0.5);
          ctx.restore();
        }
      }
    }
    ...
    if (editMode === "editNodes" && selId) {
      ...
      if (sr && sr.nodes) {
        ...
        ctx.lineWidth = 1;
        ctx.setLineDash([3, 3]);
        const [n0x, n0y] = cb(nodes[0][0], nodes[0][1]);
        ctx.beginPath();
        ctx.moveTo(n0x, n0y);
        for (let i = 1; i < nodes.length; i++) {
          const [nbx, nby] = cb(nodes[i][0], nodes[i][1]);
          ctx.lineTo(nbx, nby);
        }
        ctx.closePath();
        ctx.stroke();
        ...
        for (let i = 0; i < nodes.length; i++) {
          ...
          const midCX = (nodes[i][0] + nodes[j][0]) / 2,
            midCY = (nodes[i][1] + nodes[j][1]) / 2;
          const [mbx, mby] = cb(midCX, midCY);
          ctx.save();
          ctx.beginPath();
          ctx.arc(mbx, mby, 4, 0, Math.PI * 2);
          ...
          ctx.lineWidth = 1.2;
          ctx.beginPath();
          ctx.moveTo(mbx - 2, mby);
          ctx.lineTo(mbx + 2, mby);
          ctx.stroke();
          ctx.beginPath();
          ctx.moveTo(mbx, mby - 2);
          ctx.lineTo(mbx, mby + 2);
          ctx.stroke();
          ctx.restore();
        }
        for (let i = 0; i < nodes.length; i++) {
          ...
          ctx.save();
          ctx.beginPath();
          ctx.arc(nnx, nny, NODE_R, 0, Math.PI * 2);
          ctx.fillStyle = isActive ? ACCENT : "#fff";
          ctx.fill();
          ctx.strokeStyle = ACCENT;
          ctx.lineWidth = 2;
          ctx.stroke();
    ...
    if (isDraw.current && curPts.length > 1) {
      ...
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      const [bx0, by0] = cb(curPts[0][0], curPts[0][1]);
      ctx.beginPath();
      ctx.moveTo(bx0, by0);
      for (let i = 1; i < curPts.length; i++) {
        const [bxi, byi] = cb(curPts[i][0], curPts[i][1]);
        ctx.lineTo(bxi, byi);
      }
    ...
    if (editMode === "lasso" && lassoAnchors.length > 0) {
      ...
      if (conBuf.length > 1) {
        ...
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.moveTo(conBuf[0][0], conBuf[0][1]);
        for (let i = 1; i < conBuf.length; i++) ctx.lineTo(conBuf[i][0], conBuf[i][1]);
        ctx.stroke();
      ...
      if (lassoPreview.length > 1) {
        ...
        ctx.lineJoin = "round";
        ctx.setLineDash([6, 4]);
        const [lp0x, lp0y] = cb(lassoPreview[0][0], lassoPreview[0][1]);
        ctx.strokeStyle = "#000";
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(lp0x, lp0y);
        for (let i = 1; i < lassoPreview.length; i++) {
          const [lpx, lpy] = cb(lassoPreview[i][0], lassoPreview[i][1]);
          ctx.lineTo(lpx, lpy);
        }
        ctx.stroke();
        ctx.strokeStyle = "#00ffff";
      ...
      for (let i = 0; i < lassoAnchors.length; i++) {
        const [abx, aby] = cb(lassoAnchors[i][0], lassoAnchors[i][1]),
          isFirst = i === 0;
        ctx.save();
        if (isFirst) {
          ctx.beginPath();
          ctx.arc(abx, aby, lassoNearClose ? 12 : 9, 0, Math.PI * 2);
          ctx.strokeStyle = lassoNearClose ? "#22ff44" : "#00ffff";
          ctx.lineWidth = 2;
          ctx.stroke();
        }
        ctx.beginPath();
        ctx.arc(abx, aby, 6, 0, Math.PI * 2);
        ctx.fillStyle = isFirst && lassoNearClose ? "#22ff44" : "#00ffff";
        ctx.fill();
        ctx.strokeStyle = "#000";
        ctx.lineWidth = 1.5;
        ctx.stroke();
    ...
    const kd = e => {
      if (e.code === "Space" && !e.target.closest("input,textarea,select")) {
        ...
        if (!isPanningRef.current) {
          ...
          if (mainC.current) mainC.current.style.cursor = "grab";
        }
      }
    };
    const ku = e => {
      if (e.code === "Space") {
        ...
        if (mainC.current) mainC.current.style.cursor = "";
      }
    };
  ...
  useEffect(() => {
    ...
    const wh = e => {
      ...
      const np = {
        x: bufX - (bufX - panRef.current.x) * (newZ / zoomRef.current),
        y: bufY - (bufY - panRef.current.y) * (newZ / zoomRef.current)
      };
      const cpx = CW * 0.5 - CW * newZ,
        cpy = CH * 0.5 - CH * newZ;
      const clampedNp = {
        x: Math.max(cpx, Math.min(CW * 0.5, np.x)),
        y: Math.max(cpy, Math.min(CH * 0.5, np.y))
      };
      setZoom(newZ);
      setPan(clampedNp);
      zoomRef.current = newZ;
      panRef.current = clampedNp;
    };
  ...
  const onDown = useCallback(e => {
    ...
    if (editMode === "lasso") {
      e.preventDefault();
      if (lassoAnchors.length === 0) {
        setLassoAnchors([[mx, my]]);
        return;
      }
      if (lassoNearClose && lassoAnchors.length >= 3) {
        finishLasso();
        return;
      }
      ...
      const ax2 = Math.round(last[0]),
        ay2 = Math.round(last[1]);
      const bx2 = Math.round(mx),
        by2 = Math.round(my);
    ...
    for (let i = regions.length - 1; i >= 0; i--) if (ptIn(regions[i].points, mx, my)) {
      setSelId(regions[i].id);
      return;
    }
    setSelId(null);
  ...
  const onMove = useCallback(e => {
    ...
    if (panStateRef.current.active) {
      ...
      const np = {
        x: Math.max(CW * 0.5 - CW * cz, Math.min(CW * 0.5, rawX)),
        y: Math.max(CH * 0.5 - CH * cz, Math.min(CH * 0.5, rawY))
      };
      setPan(np);
      panRef.current = np;
      return;
    ...
    if (e.touches && e.touches.length === 2 && touchRef.current.count === 2) {
      ...
      if (tr.lastDist > 0) {
        ...
        const np0 = {
          x: midBufX - (midBufX - panRef.current.x) * (newZ / zoomRef.current),
          y: midBufY - (midBufY - panRef.current.y) * (newZ / zoomRef.current)
        };
        const panDx = (newMidX - tr.lastMidX) * sc,
          panDy = (newMidY - tr.lastMidY) * sc;
        const np = {
          x: Math.max(CW * 0.5 - CW * newZ, Math.min(CW * 0.5, np0.x + panDx)),
          y: Math.max(CH * 0.5 - CH * newZ, Math.min(CH * 0.5, np0.y + panDy))
        };
        setZoom(newZ);
        setPan(np);
        zoomRef.current = newZ;
    ...
    if (isDraw.current) {
      ...
      setCurPts(p => {
        ...
        if (!l || (pos[0] - l[0]) ** 2 + (pos[1] - l[1]) ** 2 > 4) return [...p, pos];
        return p;
      });
    }
  ...
  const onUp = useCallback(e => {
    if (panStateRef.current.active) {
      ...
      if (mainC.current) mainC.current.style.cursor = isPanningRef.current ? "grab" : "";
      return;
    }
    if (e && e.touches && e.touches.length < 2) touchRef.current = {
      count: 0,
      lastDist: 0,
      lastMidX: 0,
      lastMidY: 0
  ...
  const updateR = useCallback((id, u) => {
    setRegions(p => p.map(r => r.id === id ? {
      ...r,
      ...u
    } : r));
    setDismissed(prev => {
  ...
  const totalRecs = useMemo(() => {
    let t = 0;
    for (const r of regions) t += getRecommendations(r, regions).length;
    return t;
  }, [regions]);

  // ============ UPLOAD ============
  if (phase === "upload") return /*#__PURE__*/React.createElement("div", {
    className: "emb-page"
    ...
    onPageChange: () => {},
    setModal: setModal
  }), modal === 'help' && typeof HelpModal !== 'undefined' && /*#__PURE__*/React.createElement(HelpModal, {
    onClose: () => setModal(null)
  }), modal === 'shortcuts' && typeof ShortcutsModal !== 'undefined' && /*#__PURE__*/React.createElement(ShortcutsModal, {
    onClose: () => setModal(null)
  }), /*#__PURE__*/React.createElement("div", {
    className: "emb-container"
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
  ...
  }, "Embroidery Pattern Planner"), /*#__PURE__*/React.createElement("span", {
    style: {
    ...
    className: "emb-subtitle"
  }, "Turn images into hand embroidery patterns with stitch annotations"), /*#__PURE__*/React.createElement("div", {
  ...
  }, Icons.thread()), /*#__PURE__*/React.createElement("p", {
    className: "emb-upload-text"
  }, "Upload your own image"), /*#__PURE__*/React.createElement("input", {
    ref: fileRef,
    type: "file",
    accept: "image/*",
    onChange: handleFile,
    ...
    className: "emb-btn emb-btn--primary",
    onClick: e => {
      e.stopPropagation();
      fileRef.current?.click();
    }
  }, "Upload Image"))), /*#__PURE__*/React.createElement("canvas", {
    ref: imgC,
    style: {
      display: "none"
    }
  ...
  // ============ SEGMENT ============
  if (phase === "segment") return /*#__PURE__*/React.createElement("div", {
    className: "emb-page"
  }, /*#__PURE__*/React.createElement(Header, {
    page: "embroidery",
    tab: null,
    ...
    className: "emb-nav-row"
  }, /*#__PURE__*/React.createElement("button", {
    className: "emb-back-btn",
    "aria-label": "Back",
    onClick: () => {
      setPhase("upload");
      setImgSrc(null);
  ...
  }, Icons.chevronLeft())), /*#__PURE__*/React.createElement("h2", {
    className: "emb-heading"
    ...
    src: imgSrc,
    alt: "",
    className: "emb-preview-img"
  })), /*#__PURE__*/React.createElement("div", {
    className: "emb-method-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: "emb-method-card emb-method-card--auto"
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: "center",
  ...
  }, Icons.sparkles()), /*#__PURE__*/React.createElement("h3", {
    className: "emb-method-title"
  }, "Auto Detect"), /*#__PURE__*/React.createElement("p", {
    className: "emb-method-desc"
  }, "SLIC superpixel segmentation")), /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", {
    className: "emb-label"
  }, "Colour regions: ", numColors), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: 4,
    max: 16,
    value: numColors,
    ...
    className: "emb-range-labels"
  }, /*#__PURE__*/React.createElement("span", null, "Fewer"), /*#__PURE__*/React.createElement("span", null, "More"))), /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", {
    className: "emb-label"
  }, "Edge sensitivity: ", compactness <= 5 ? 'High' : compactness <= 14 ? 'Medium' : 'Low'), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: 1,
    max: 20,
    value: 21 - compactness,
    onChange: e => setCompactness(21 - +e.target.value),
    style: {
      width: "100%"
    }
  }), /*#__PURE__*/React.createElement("div", {
    className: "emb-range-labels"
  }, /*#__PURE__*/React.createElement("span", null, "Low"), /*#__PURE__*/React.createElement("span", null, "High"))), /*#__PURE__*/React.createElement("button", {
    className: "emb-btn emb-btn--primary emb-btn--full",
    onClick: runAuto,
    disabled: loading,
    style: {
      opacity: loading ? .6 : 1,
  ...
  }, loading ? "Detecting…" : "Auto Segment")), /*#__PURE__*/React.createElement("div", {
    className: "emb-method-card emb-method-card--draw"
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: "center"
    }
  ...
  }, Icons.pencil()), /*#__PURE__*/React.createElement("h3", {
    className: "emb-method-title"
  }, "Draw by Hand"), /*#__PURE__*/React.createElement("p", {
    className: "emb-method-desc"
  }, "Freehand trace regions")), /*#__PURE__*/React.createElement("button", {
    className: "emb-btn emb-btn--secondary emb-btn--full",
    onClick: startDraw,
    style: {
      marginTop: 'var(--s-3)'
    }
  }, "Start Drawing")))), /*#__PURE__*/React.createElement("canvas", {
    ref: imgC,
    style: {
      display: "none"
    }
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "emb-heading"
  }, "Edit Pattern")), totalRecs > 0 && editMode === "select" && /*#__PURE__*/React.createElement("div", {
    className: "emb-suggestion-badge"
  }, totalRecs, " suggestion", totalRecs > 1 ? "s" : "")), /*#__PURE__*/React.createElement("div", {
    className: "tb-grp",
    style: {
      width: "100%",
      marginBottom: 6
    ...
    className: 'tb-btn' + (editMode === "select" ? ' tb-btn--on' : ''),
    onClick: () => {
      setEditMode("select");
      setDragNode(null);
    }
  ...
  }, Icons.pointing(), " Select")), sel && /*#__PURE__*/React.createElement("button", {
    className: 'tb-btn' + (isNodeEdit ? ' tb-btn--on' : ''),
    onClick: () => {
      setEditMode("editNodes");
      setDragNode(null);
    }
  }, /*#__PURE__*/React.createElement("span", {
    style: {
  ...
  }, Icons.nodes(), " Edit Shape")), /*#__PURE__*/React.createElement("button", {
    className: 'tb-btn' + (editMode === "draw" ? ' tb-btn--on' : ''),
    onClick: () => {
      setEditMode("draw");
      setSelId(null);
      setDragNode(null);
  ...
  }, Icons.pencil(), " Add")), /*#__PURE__*/React.createElement("button", {
    className: 'tb-btn' + (editMode === "wand" ? ' tb-btn--on' : ''),
    onClick: () => {
      setEditMode("wand");
      setSelId(null);
      setDragNode(null);
  ...
  }, Icons.wand(), " Wand")), /*#__PURE__*/React.createElement("button", {
    className: 'tb-btn' + (editMode === "lasso" ? ' tb-btn--on' : ''),
    onClick: () => {
      setEditMode("lasso");
      setSelId(null);
      setDragNode(null);
  ...
  }, Icons.lasso(), " Lasso"))), /*#__PURE__*/React.createElement("div", {
    className: "tb-grp",
    style: {
      width: "100%",
      marginBottom: 6,
    ...
    className: "tb-btn",
    onClick: () => {
      const nz = Math.max(ZOOM_MIN, zoom - ZOOM_STEP_BUTTON);
      const np = {
        x: CW * (1 - nz) / 2,
        y: CH * (1 - nz) / 2
      };
    ...
    className: "tb-btn",
    onClick: () => {
      const nz = Math.min(ZOOM_MAX, zoom + ZOOM_STEP_BUTTON);
      const np = {
        x: CW * (1 - nz) / 2,
        y: CH * (1 - nz) / 2
      };
  ...
  }, "Fit")), editMode === "draw" && /*#__PURE__*/React.createElement("div", {
    className: "emb-hint emb-hint--teal"
  }, "Drag on the canvas to draw a new region."), editMode === "wand" && /*#__PURE__*/React.createElement("div", {
    className: "emb-hint emb-hint--teal",
    style: {
      marginBottom: 6
    }
  }, "Click anywhere on the image to fill a colour region.", /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 6
    }
  }, /*#__PURE__*/React.createElement("label", {
    className: "emb-label"
  }, "Tolerance: ", wandTolerance), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: 0,
    max: 100,
    value: wandTolerance,
    ...
    className: "emb-range-labels"
  }, /*#__PURE__*/React.createElement("span", null, "Exact"), /*#__PURE__*/React.createElement("span", null, "Loose"))), /*#__PURE__*/React.createElement("label", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: 6,
    ...
    type: "checkbox",
    checked: wandEdgeSnap,
    onChange: e => setWandEdgeSnap(e.target.checked)
  }), "Edge snap")), editMode === "lasso" && /*#__PURE__*/React.createElement("div", {
    className: "emb-hint emb-hint--teal",
    style: {
      marginBottom: 6
    }
  }, /*#__PURE__*/React.createElement("strong", null, "Click"), " to place anchors \u2014 path snaps to edges. Move near first anchor to close and fill the region. ", /*#__PURE__*/React.createElement("strong", null, "Ctrl+Z"), " to undo."), isNodeEdit && /*#__PURE__*/React.createElement("div", {
    className: "emb-hint emb-hint--amber"
  }, /*#__PURE__*/React.createElement("strong", null, "Drag nodes"), " to reshape. Click a ", /*#__PURE__*/React.createElement("strong", null, "+"), " midpoint or any edge to add a node. Double-click a node to remove it (min 3)."), /*#__PURE__*/React.createElement("div", {
    className: "tb-grp",
    style: {
      width: "100%",
      marginBottom: 6
    }
  }, ["overlay", "stitch", "original"].map(v => /*#__PURE__*/React.createElement("button", {
    key: v,
    className: 'tb-btn' + (view === v ? ' tb-btn--on' : ''),
    onClick: () => setView(v),
    style: {
      flex: 1,
    ...
    onTouchEnd: onUp,
    onDoubleClick: e => {
      if (editMode === "lasso" && lassoAnchors.length >= 3) {
        finishLasso();
        return;
      }
    ...
    style: {
      width: "100%",
      display: "block",
      cursor: editMode === "draw" || editMode === "wand" || editMode === "lasso" ? "crosshair" : isNodeEdit ? "default" : "pointer"
    }
  })), sel && editMode !== "draw" ? /*#__PURE__*/React.createElement("div", {
    className: "card",
    style: {
      padding: 12,
      marginBottom: 'var(--s-2)'
    ...
    className: "emb-color-swatch",
    style: {
      background: sel.dmc.h
    }
  }), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("input", {
    value: sel.label,
    onChange: e => updateR(sel.id, {
      label: e.target.value
    }),
    className: "emb-region-name-input"
  }), /*#__PURE__*/React.createElement("div", {
    className: "emb-region-dmc"
  }, "DMC ", sel.dmc.c, " \u2014 ", sel.dmc.n))), /*#__PURE__*/React.createElement("button", {
    className: "emb-delete-btn",
    "aria-label": "Delete region",
    onClick: () => deleteR(sel.id)
  }, /*#__PURE__*/React.createElement("span", {
    style: {
      display: "inline-flex",
      alignItems: "center",
  ...
  }, Icons.x()))), isNodeEdit && /*#__PURE__*/React.createElement("div", {
    className: "emb-hint emb-hint--amber",
    style: {
      marginBottom: 'var(--s-2)'
    }
  }, sel.nodes.length, " control nodes"), !isNodeEdit && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 'var(--s-2)'
    }
  }, /*#__PURE__*/React.createElement("label", {
    className: "emb-label"
  }, "Stitch Type"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexWrap: "wrap",
      gap: 3
    }
  }, STITCHES.map(st => /*#__PURE__*/React.createElement("button", {
    key: st.id,
    onClick: () => updateR(sel.id, {
      stitch: st.id
    }),
    className: 'emb-stitch-btn' + (sel.stitch === st.id ? ' emb-stitch-btn--active' : ''),
    style: sel.stitch === st.id ? {
      background: st.color,
      borderColor: st.color,
      color: "var(--surface)"
    } : undefined
  }, st.name))), /*#__PURE__*/React.createElement("p", {
    className: "emb-stitch-desc"
  }, STITCHES.find(s => s.id === sel.stitch)?.desc)), selRecs.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 'var(--s-2)'
    }
  }, selRecs.map((rec, i) => {
    const isW = rec.type === "warning";
    return /*#__PURE__*/React.createElement("div", {
      key: i,
      className: 'emb-rec' + (isW ? ' emb-rec--warn' : ' emb-rec--tip'),
      style: {
        marginBottom: i < selRecs.length - 1 ? 4 : 0
      }
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
    ...
    }, Icons[rec.icon] ? Icons[rec.icon]() : null), rec.msg), /*#__PURE__*/React.createElement("div", {
      className: "emb-rec-suggest"
    }, rec.suggest)), /*#__PURE__*/React.createElement("button", {
      className: "emb-rec-dismiss",
      "aria-label": "Dismiss",
      onClick: () => setDismissed(p => new Set(p).add(`${sel.id}-${rec.msg.slice(0, 20)}`))
    }, /*#__PURE__*/React.createElement("span", {
      "aria-hidden": "true",
      style: {
        display: 'inline-flex'
      }
    }, Icons.x ? Icons.x() : "\u00D7"))), rec.fix && /*#__PURE__*/React.createElement("button", {
      className: 'emb-btn emb-btn--sm' + (isW ? ' emb-btn--amber' : ' emb-btn--primary'),
      onClick: () => updateR(sel.id, rec.fix),
      style: {
        marginTop: 6,
        display: 'inline-flex',
        alignItems: 'center',
    ...
    }, /*#__PURE__*/React.createElement("span", null, "Apply"), rec.fix.stitch && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
      "aria-hidden": "true",
      style: {
        display: 'inline-flex'
      }
    }, Icons.chevronRight()), /*#__PURE__*/React.createElement("span", null, STITCHES.find(s => s.id === rec.fix.stitch)?.name || "")), rec.fix.direction !== undefined && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
      "aria-hidden": "true",
      style: {
        display: 'inline-flex'
      }
    }, Icons.chevronRight()), /*#__PURE__*/React.createElement("span", null, rec.fix.direction, "\xB0"))));
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 'var(--s-2)'
    }
    ...
    className: "emb-label"
  }, "Direction: ", sel.direction, "\xB0"), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: 0,
    max: 359,
    value: sel.direction,
    ...
    className: "emb-range-labels"
  }, /*#__PURE__*/React.createElement("span", null, "0\xB0 \u2192"), /*#__PURE__*/React.createElement("span", null, "90\xB0 \u2193"), /*#__PURE__*/React.createElement("span", null, "180\xB0 \u2190"), /*#__PURE__*/React.createElement("span", null, "270\xB0 \u2191"))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    className: "emb-label"
  }, "DMC Thread"), /*#__PURE__*/React.createElement("div", {
    className: "emb-dmc-grid"
  }, EMB_COLORS.filter(c => {
    const [cr, cg, cb] = hex2rgb(c.h);
    return (sel.avgColor[0] - cr) ** 2 + (sel.avgColor[1] - cg) ** 2 + (sel.avgColor[2] - cb) ** 2 < 28000;
  }).slice(0, 14).map(c => /*#__PURE__*/React.createElement("button", {
    key: c.c,
    onClick: () => updateR(sel.id, {
      dmc: c
    }),
    title: `DMC ${c.c} — ${c.n}`,
    className: 'emb-dmc-swatch' + (sel.dmc.c === c.c ? ' emb-dmc-swatch--active' : ''),
    style: {
      background: c.h
    }
  })))))) : editMode === "select" && /*#__PURE__*/React.createElement("p", {
    className: "emb-hint-text"
  }, "Tap a region to edit"), regions.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "card",
    style: {
      padding: 10
    }
  }, /*#__PURE__*/React.createElement("h3", {
    className: "emb-legend-title"
  }, "Legend \u2014 ", regions.length, " region", regions.length > 1 ? "s" : ""), /*#__PURE__*/React.createElement("div", {
    className: "emb-legend-grid"
  }, regions.map(r => {
    const recs = getRecommendations(r, regions).filter(rc => !dismissed.has(`${r.id}-${rc.msg.slice(0, 20)}`));
    const hw = recs.some(rc => rc.type === "warning");
    return /*#__PURE__*/React.createElement("div", {
      key: r.id,
      className: 'emb-legend-item' + (selId === r.id ? ' emb-legend-item--active' : ''),
      onClick: () => {
        if (editMode !== "draw") {
          setSelId(r.id);
          if (editMode === "editNodes" && r.id !== selId) setEditMode("select");
        }
      }
    }, /*#__PURE__*/React.createElement("span", {
      className: "emb-legend-dot",
      style: {
        background: r.dmc.h
      }
    }), /*#__PURE__*/React.createElement("div", {
      ...
      className: "emb-legend-name"
    }, r.label), /*#__PURE__*/React.createElement("div", {
      className: "emb-legend-meta"
    }, r.dmc.c, " \xB7 ", STITCHES.find(s => s.id === r.stitch)?.name, " \xB7 ", r.direction, "\xB0")), recs.length > 0 && /*#__PURE__*/React.createElement("span", {
      className: 'emb-legend-badge' + (hw ? ' emb-legend-badge--warn' : '')
    }));
  }))), /*#__PURE__*/React.createElement("canvas", {
    ref: imgC,
    style: {
...
root.render(/*#__PURE__*/React.createElement(EmbroideryApp, null));
})();
