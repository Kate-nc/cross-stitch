...
function handleFractionalClick(cell, newComp, conflictAction = "replace") {
  ...
  if (cell && cell.type === "solid" && cell.id !== "__skip__") {
     if (cell.id === newComp.id) return { changed: false, cell: cell };
     // solid is equivalent to 2 half stitches or 4 quarters, but for simplicity,
     // if a user clicks a solid to place a fraction, we can prompt or just replace.
     // By standard logic, placing fraction on solid replaces it.
  ...
  for (let i = 0; i < components.length; i++) {
    let c = components[i];
    let ck = pathKey(c.path.start, c.path.end);
    if (nk === ck && c.type === newComp.type && c.id === newComp.id) {
       exactMatchIdx = i;
       break;
    }
  }
