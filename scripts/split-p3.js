// Extract P3 items from master TODO into per-area files for subagent dispatch.
const fs = require('fs');
...
for (const l of lines) {
  if (/^##\s*P3\b/.test(l)) { inP3 = true; continue; }
  if (inP3 && /^##\s/.test(l)) break;
  if (inP3 && /^\s*-\s*\[\s*\]/.test(l)) items.push(l);
}
...
const outDir = 'reports/verification/p3-batches';
