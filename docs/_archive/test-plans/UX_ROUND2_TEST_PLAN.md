...
Manual QA checklist for the second consolidation pass. Test each phase in order. **Important:** before testing, hard-refresh each page (Ctrl + Shift + R) once to pick up the current hashed Creator cache key (now generated as `babel_creator_<hash>` by `build-creator-bundle.js`) and the new `preferences-modal.js` script.
...
1. Hard-refresh each of the three pages once. Watch the console.
   - Expect: `Cached compiled creator code under key babel_creator_<hash>` (Creator only). The hash is auto-generated by `build-creator-bundle.js` from the bundle contents.
...
- ✅ Cache key: now content-hashed (e.g. `babel_creator_<hash>`); auto-bumped by `build-creator-bundle.js`.
